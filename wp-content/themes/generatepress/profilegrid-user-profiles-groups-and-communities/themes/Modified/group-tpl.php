    ...
    <?php if($show_group_card==1):?>
       <div class="pm-group-card-box pm-dbfl pm-border-bt">
         <div class="pm-group-card pm-dbfl pm-border pm-bg pm-radius5">
			...
			?>
           <div class="pm-edit-group"><a href="<?php echo esc_url( $edit_group ); ?>" class="pm_button"><?php _e('Edit','profile-magic');?></a></div>
            <?php endif;?>
            </div>
             ...
             <div class="pm-group-description pm-difl pm-bg pm-pad10 pm-border">
         		...
         		<?php 
                            if(!class_exists('Profilegrid_Group_Multi_Admins')):                         
                                ...
                                <?php echo get_avatar($leaders['primary'],16,'',false,array('class'=>'pm-infl'));?>
        		...
        		<?php   endif;
                 ...
                 <div class="pm-card-row pm-dbfl">
                    <div class="pm-card-label pm-difl"><?php _e('Members','profile-magic');?></div>
                    <div class="pm-card-value pm-difl"><?php echo $total_users?></div>
                 </div>
                 
                 <?php if(!empty($row->group_desc)):?>
                  <div class="pm-card-row pm-dbfl">
                    <div class="pm-card-label pm-difl"><?php _e('Details','profile-magic');?></div>
   ...
   if($requested!=null)
    {
       ...
       _e(sprintf('You sent membership request to join this Group on %s',$requested_options['request_date']),'profile-magic'); ?>
    <?php if(!empty($leaders)):
        if(isset($leaders['primary'])){$group_admin_id = $leaders['primary'];}else{$group_admin_id = $leaders[0];}
        ?>
    <br />
       <a onclick="pg_edit_blog_popup('member','message','<?php echo $group_admin_id;?>','<?php echo $gid;?>')"><?php echo sprintf(__("Send a message to %s","profile-magic"),$pmrequests->pm_get_group_admin_label($gid));?></a>
           <?php
           endif;
       echo '</div></div>';
    }
    ...
    <div id="pg_group_tabs" class="pm-section-nav-horizental pm-dbfl">
        <ul class="pm-difl pm-profile-tab-wrap pm-border-bt">	
            ...
            <li class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg_members"><?php _e('Members','profile-magic');?></a></li>
...
<?php
        ...
        else
        {
            ...
            _e('No User Profile is registered in this Group','profile-magic');
            echo '</div>';
        }
