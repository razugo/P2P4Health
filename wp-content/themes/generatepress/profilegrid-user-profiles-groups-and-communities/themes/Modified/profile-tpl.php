  ...
  <div class="pm-group-view pm-dbfl">
    <div class="pm-header-section pm-dbfl pm-bg pm-border pm-radius5"> 
      ...
      <div class="pm-cover-image pm-dbfl" <?php if($uid != $current_user->ID) echo 'id="pm-show-cover-image"';?> > 
        ...
        <div class="pm-bg-dk pg-profile-change-img dbfl" id="pm-coverimage-mask">
              <div id="pm-change-cover-image" class="pg-item-image-change">
                  ...
                  <?php _e('Update Cover Image','profile-magic');?>
              </div>
          </div>
        <?php endif;?>
      </div>
      ...
      <div class="pm-profile-title-header pm-dbfl">
        <div  <?php if($uid != $current_user->ID) echo 'id="pm-show-profile-image"';?> class="pm-profile-image pm-difl pm-pad10"> <?php echo get_avatar($user_info->user_email,150,'',false,array('class'=>'pm-user'));?>
          <?php if($uid == $current_user->ID):?>
         <div class="pm-bg-dk pg-profile-change-img">
            <div id="pm-change-image" class="pg-item-image-change">
                <i class="fa fa-camera-retro" aria-hidden="true"></i>
                <?php _e('Update Image','profile-magic');?></div>
          </div>
          <?php endif;?>
        </div>
        <div class="pm-profile-title pm-difl pm-pad10">
          ...
          <div class="pm-user-group-name pm-dbfl pm-clip">
              ...
              <?php if($total_assign_group>2){ $group_count_String = __('more groups','profile-magic');}else{$group_count_String = __('more group','profile-magic');} ?>
    ...
    <div class="pm-profile-tabs pm-dbfl" id="pg-profile-tabs">
     <div class="pm-section-nav-horizental pm-dbfl">
        ...
        <ul class="mymenu pm-difl pm-profile-tab-wrap pm-border-bt" >	
            ...
            <li class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg-messages" onClick="refresh_messenger();"><?php _e('Messages','profile-magic');?><b id="unread_thread_count" class=""></b></a></li>
            ...
            ?>            
            <li id="notification_tab" onclick="read_notification();" class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg-notifications"><?php _e('Notifications','profile-magic');?><b id="unread_notification_count" class="<?php echo $unread_notification_class; ?>"><?php echo $unread_notification;?></b></a></li>
            <?php
            } ?>
	...
	?>
      ...
      <div class="pm-edit-user pm-difl pm-pad10"> <a href="<?php echo esc_url( $redirect_url ); ?>" class="pm-dbfl">
          ...
          <?php _e('Edit Profile','profile-magic');?></a> </div>
      </div>
      <?php endif; ?>
	  ...
	  ?>
        ...
        <div id="pg-blog" class="pm-difl pg-profile-tab-content">
            ...
            <div class="pg-new-blog-button pm-border">
                <a <?php echo $string;?>><?php _e('New Blog Post','profile-magic');?></a>
            </div>
            </div>
            <?php endif;?>
  ...
  <div class="pm-popup-mask"></div>
    ...
    <div class="pm-popup-container pm-update-image-container pm-radius5">
      <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
          <i class="fa fa-camera-retro" aria-hidden="true"></i>
        <?php _e('Change Profile Image','profile-magic');?>
      ...
      <div class="pm-popup-image pm-dbfl pm-bg pm-pad10"> 
        ...
        <div class="pm-popup-action">
          <a type="button" class="btn btn-primary" id="change-pic"><?php _e('Change Image','profile-magic');?></a>
	  <div id="changePic" class="" style="display:none">
            <form id="cropimage" method="post" enctype="multipart/form-data" action="<?php echo admin_url( 'admin-ajax.php' );?>">
                <div class="pm-dbfl">
	           <label><?php _e('Upload your image','profile-magic');?></label>
	    ...
	    <div id="thumbs" style="padding:5px; width:600px"></div>	
            ...
            <div class="modal-footer">
                <button type="button" id="btn-cancel" class="btn btn-default"><?php _e('Cancel','profile-magic');?></button>
                <button type="button" id="btn-crop" class="btn btn-primary"><?php _e('Crop & Save','profile-magic');?></button>
            </div>
          </div>
          <form method="post" action="" enctype="multipart/form-data">
            ...
            <input type="submit" value="<?php _e('Remove','profile-magic');?>" name="remove_image" />
          </form>
        </div>
        <p class="pm-popup-info pm-dbfl pm-pad10">
          <?php _e('For best visibility choose square image with minimum size of 200 x 200 pixels','profile-magic');?>
        </p>
      </div>
    </div>
  </div>
<div class="pm-popup-mask"></div>
  <div id="pm-change-cover-image-dialog">
    <div class="pm-popup-container pm-update-image-container pm-radius5">
      <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
        <?php _e('Change Cover Image','profile-magic');?>
          <div class="pm-popup-close pm-difr">
              <img src="<?php echo $path;?>images/popup-close.png" height="24px" width="24px">
          </div>
      </div>
      <div class="pm-popup-image pm-dbfl pm-pad10 pm-bg"> 
        ...
        <div class="pm-popup-action pm-dbfl pm-pad10">
          <a type="button" class="btn btn-primary" id="change-cover-pic"><?php _e('Change Cover Image','profile-magic');?></a>
	  <div id="changeCoverPic" class="" style="display:none">
	    ...
	    <label><?php _e('Upload Your Cover Image','profile-magic');?></label>
	    ...
	    <div id="thumbs" style="padding:5px; width:600px"></div>	
            ...
            <div class="modal-footer">
                <button type="button" id="btn-cover-cancel" class="btn btn-default"><?php _e('Cancel','profile-magic');?></button>
                <button type="button" id="btn-cover-crop" class="btn btn-primary"><?php _e('Crop & Save','profile-magic');?></button>
            </div>
          </div>
        ...
        <p class="pm-popup-info pm-dbfl pm-pad10">
          <?php _e('For best visibility choose a landscape aspect ratio image with size of <span id="pm-cover-image-width">1200</span> x 300 pixels','profile-magic');?>
        </p>
      </div>
    </div>
  </div>
          ...
          <div id="pm-add-blog-dialog" style="<?php if(isset($_POST['pg_blog_submit']))echo 'display:block';?>">
            <div class="pm-popup-container pm-radius5">
              <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                ...
                <?php _e('Submit New Blog Post','profile-magic');?>
...
<div class="pm-popup-mask"></div>    
    ...
    <div id="pm-show-profile-image-dialog">
        <div class="pm-popup-container">
            ...
            <div class="pm-popup-image pm-dbfl pm-pad10 pm-bg">    
                <?php echo get_avatar($user_info->user_email, 512); ?>
            </div>
