...
if ( is_user_logged_in()) : ?>
	...
	?> 
	  <div class="pm-login-header pm-dbfl pm-bg pm-border-bt pm-pad10">
		  <h4><?php _e( "You can reset your password by accessing Change Password tab in your profile's Settings section.",'profile-magic' );?></h4>
...
?>
 ...
 <form id="lostpasswordform" class="pmagic-form pm-dbfl pm-bg-lt" action="<?php echo site_url('/wp-login.php?action=lostpassword'); ?>" method="post">
     <div class="pm-login-header pm-dbfl pm-bg pm-border-bt pm-pad10"><?php _e('If you have lost your password, we can help you reset it. To initiate password reset process, please enter your username or email in the box below and click on the button.','profile-magic');?></div>
     <input type="text" name="<?php echo esc_attr('user_login');?>" id="user_login" placeholder="<?php _e('Email or Username','profile-magic');?>">
         
    
            <div class="pm-login-box-bottom-container pm-dbfl pm-bg pm-border">
                <input type="submit" name="submit" class="lostpassword-button" value="<?php _e( 'Reset Password','profile-magic' ); ?>"/>
            </div>
  </form>
 <?php endif;?>
 </div>
