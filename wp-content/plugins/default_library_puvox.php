 ...
 * We use symlinks for easy changes across all plugins : https://puvox.software/blog/easiest-method-create-symlink-symbolic/ 
 *
 * @package   Puvox.software plugin
...
if(!trait_exists('default_methods__Puvox_Software')) 
{
  trait default_methods__Puvox_Software
  {
		public function __construct($arg1=false)
		{
			...
			$this->is_development 	= defined("_puvox_machine_") ;			// set in my_superglobals.php in devmachine
		...
		public function setupLinksAndMenus()
		{
			...
			add_filter( (is_network_admin() ? 'network_admin_' : ''). 'plugin_action_links_'.plugin_basename($this->plugin_FILE),  function($links){
				...
				if($this->static_settings['show_opts']){ $links[] = '<a href="'.$this->plugin_page_url_current.'">'.$this->static_settings['menu_text']['settings'].'</a>';  }
		...
		*/
		...
		public function my_plugin_vars($step=0)
		{
			...
			$domain ='https://puvox.software' ; // $this->is_development && !property_exists($this,'avoid_localhost_development') ? 'https://puvox.software' ://https://127.0.0.1/wp/puvox.software
		...
		public function update_transient_CHOSEN($optname, $optvalue, $autoload=null		, $prefix=false){
			return call_user_func("set_".		( $this->newtork_managed ? "site_" : "" ). "transient",  $this->optName($optname, $prefix), $optvalue, $autoload );
		}
		...
		public function delete_transients_by_prefix_CHOSEN($myPrefix					, $prefix=false){
			...
			$table_name		= $this->newtork_managed ? $wpdb->base_prefix.'sitemeta' : $wpdb->prefix .'options' ;
			$column_name	= $this->newtork_managed ? 'meta_key' : 'option_name';
		...
		public function is_administrator()	{ if (function_exists('current_user_can') || require_once(ABSPATH.'wp-includes/pluggable.php')) return (current_user_can('install_plugins')); return false; }
		...
		*/
		...
		//i.e.  123424235.325434645
		public function decimal_outputer($input, $length=4, $only_dot=false){  
			$timeParts = explode('.', $input);
			return ($only_dot ? '' : $timeParts[0] . '.') . substr($timeParts[1], 0, $length); //sprintf('%.4F',$input); 
		}
		...
		public function opts_page_output_parent($args=false)
		{
			...
			{ 
				...
				<form method="post" action="">
					<?php _e("Choose, how you wish to manage this plugin settings"); ?><br/>
					<?php _e("Network-Wide");?>:<input onchange="managed_from_onchanger(this)" type="radio" name="<?php echo $this->slug;?>[managed_from_site]" value="network" <?php checked($this->newtork_managed);?> /> &nbsp; &nbsp;
					<?php _e("Per sub-site, individually");?>:<input onchange="managed_from_onchanger(this)"  type="radio" name="<?php echo $this->slug;?>[managed_from_site]" value="singlesite" <?php checked(!$this->newtork_managed);?>  />
		...
		public function admin_error_notice_pro(){ ?>
			<div class="notice notice-error is-dismissible">
				<p><?php  _e( sprintf('Notice: License for plugin <code><b>%s</b></code> is invalidated, so it\'s <b style="color:red;">PRO</b> functionality has been disabled.', $this->static_settings['Name']) );?> <a href="<?php echo $this->plugin_page_url_current;?>" target="_blank"><?php _e("Re-validate the key");?></a></p> 
			</div>
			<?php
		}
