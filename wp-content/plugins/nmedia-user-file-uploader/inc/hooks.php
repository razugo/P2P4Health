...
function wpfm_hooks_after_file_saved( $wpfm_post_id, $file_data, $user_id ) {
	...
	$file_url		= wpfm_get_file_dir_url($user_id).$filename;
	...
	if( $is_image ) {
		$file_thumb_url		= wpfm_get_file_dir_url($user_id, true).$filename;
	...
	*/
	...
	if ( wpfm_is_amazon_addon_enable()	&& isset($file_data['amazon']) ) {
		...
		$post_attachment_url 	= wpfm_get_file_dir_url($user_id) . $file_data['filename'];
