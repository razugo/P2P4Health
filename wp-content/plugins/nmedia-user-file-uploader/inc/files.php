...
function wpfm_save_file_data() {
	...
	$file_objects = wpfm_save_uploaded_transferred_files( $current_user->ID, $all_files_with_data );
...
function wpfm_save_uploaded_transferred_files( $user_id, $wpfm_files ) {
	
	$file_objects = array();
	
	foreach( $wpfm_files as $key => $file_data){
		// var_dump($file_data['title']);
...
function wpfm_get_wp_files( $parent_id = 0 ) {
    ...
    // Important filter: will pull files for different actions
    // shared, group etc
...
function wpfm_file_meta_update() {
	...
	echo __("File Meta save successfully", "wpfm");
	die(0);
}
