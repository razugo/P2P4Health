...
function wpfm_array_fileapi_vars () {
	...
	$messages_js = array('files_loading'	=> __('Files are being loaded ...', 'wpfm'),
		 						...
		 						'file_uploaded' => __('File is uploaded successfully', 'wpfm'),
	...
	$wpfm_js_vars = array(
			...
			'max_file_size' 	=> wpfm_get_option('_max_file_size') != '' ? wpfm_get_option('_max_file_size') : '2mb',
			'max_files' 		=> (wpfm_get_option('_max_files') == '') ? 5 : wpfm_get_option('_max_files'),
			'max_files_message' => sprintf(__("Max. %d file(s) can be uploaded", 'wpfm'), wpfm_get_option('_max_files')),
...
function wpfm_get_users_directories($user_id) {
	
	$current_user = is_null($user_id) ? wpfm_get_current_user() : get_userdata( $user_id );
	$dir_map = array('root'		=> $current_user->user_login,
		'thumbs'	=> $current_user->user_login.'/thumbs',
		'ftp'		=> $current_user->user_login.'/ftp');
