...
//put your code here
	...
	{
		add_meta_box( 'profile-magic-access-metabox', __( 'ProfileGrid','profile-magic'),array( $this, 'pm_display_meta_box' ), 'page');
		add_meta_box( 'profile-magic-access-metabox', __( 'ProfileGrid','profile-magic'),array( $this, 'pm_display_meta_box' ), 'post');
                add_meta_box( 'profile-magic-access-metabox', __( 'ProfileGrid','profile-magic'),array( $this, 'pm_display_meta_box' ), 'profilegrid_blogs');
                add_meta_box( 'profile-magic-access-metabox', __( 'ProfileGrid','profile-magic'),array( $this, 'pm_display_meta_box' ), 'pg_groupwall');
	}
			...
			if(get_post_meta($id,'pm_content_access',true)==2)
			{
				  if ( is_user_logged_in() ) 
				  {
					...
					if(get_post_meta($id,'pm_content_access_group',true)!='all')
					{
						...
						{
							 $error = $pmrequests->profile_magic_get_error_message('not_permitted','profile-magic');
				  ...
				  {
					  $error = $pmrequests->profile_magic_get_error_message('loginrequired','profile-magic');
					  $content = $this->profile_magic_content_access_message($error);
				  }
			}
