...
if($user_info==false)
{
 echo '<div class="pmagic"><div class="pm_message">'.__('This user no longer exists.','profile-magic').'</div></div>';
}
else {
?>
<div class="pmagic"> 
  ...
  <div class="operationsbar">
    ...
    <div class="nav">
      <ul>
        <li><a href="user-edit.php?user_id=<?php echo $id;?>"><?php _e("Edit",'profile-magic');?></a></li>
        <?php if ($id != $current_user->ID ){ ?>
        <li><input type="submit" name="delete" value="<?php _e('Delete','profile-magic');?>" onclick="return pg_confirm('<?php _e('Are you sure you want to delete this user permanently? You cannot undo this action.','profile-magic');?>')" /></li>
        <?php if($pmrequests->profile_magic_get_user_field_value($id,'rm_user_status')==1): ?>
        <li><input type="submit" name="activate" value="<?php _e("Activate",'profile-magic');?>" /></li>
        <?php else : ?>
        <li><input type="submit" name="deactivate" value="<?php _e("Deactivate",'profile-magic');?>" /></li>
        <?php endif; }?>
  ...
  <div class="pm-user-area">
    <div class="pm-user-info">
      ...
      <div class="pm-profile-fields">
        <div class="pm-profile-field-row">
          <div class="pm-field-label"><?php _e('First Name','profile-magic');?></div>
          <div class="pm-field-value"><?php echo $user_info->first_name;?></div>
        </div>
        <div class="pm-profile-field-row">
          <div class="pm-field-label"><?php _e('Last Name','profile-magic');?></div>
          <div class="pm-field-value"><?php echo $user_info->last_name;?></div>
        </div>
        <div class="pm-profile-field-row">
          <div class="pm-field-label"><?php _e('Email','profile-magic');?></div>
          <div class="pm-field-value"><?php echo $user_info->user_email;?></div>
        </div>
        <div class="pm-profile-field-row">
          <div class="pm-field-label"><?php _e('Role','profile-magic');?></div>
          <div class="pm-field-value"><?php echo $userrole;?></div>
        </div>
        <div class="pm-profile-field-row">
          <div class="pm-field-label"><?php _e('Bio','profile-magic');?></div>
	...
	?>
      ...
      <div class="pm-profile-fields">
        ...
        <?php
                ...
                ?>
          <div class="pmnotice"><?php _e('No User Profile Fields in this section.','profile-magic');?></div>
          <?php
            endif;
	?>
      </div>
    ...
    <?php 
        ...
        echo '<div class="pm_message">'.__('This profile is not yet associated with any parent profile group. To make it visible please associate it with a group first.','profile-magic').'</div>';
    endif;
    ?>
