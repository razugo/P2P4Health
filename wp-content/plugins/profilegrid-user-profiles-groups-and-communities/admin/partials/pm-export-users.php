...
if(filter_input(INPUT_POST,'export_users'))
{
    ...
    if (!wp_verify_nonce($retrieved_nonce, 'pm_export_users' ) ) die( __('Failed security check','profile-magic') );
...
<div class="uimagic">
  <form name="pm_export_users" id="pm_export_users" method="post">
    ...
    <div class="content">
      <div class="uimheader">
        <?php _e( 'Export Users','profile-magic' ); ?>
      </div>
      ...
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Select Group(s):','profile-magic' ); ?>
        </div>
        ...
        <div class="uimnote"><?php _e('Step 1: Select the Group(s) from which you wish to export users. Press ctrl on Windows/Linux PC and ⌘ on Mac while selecting to choose multiple Groups.','profile-magic');?></div>
      </div>
     ...
     <div class="uimrow">
      ...
      <div class="buttonarea"> <a href="admin.php?page=pm_tools">
        ...
        <input type="submit" value="<?php _e('Export','profile-magic');?>" name="export_users" id="export_users" />
