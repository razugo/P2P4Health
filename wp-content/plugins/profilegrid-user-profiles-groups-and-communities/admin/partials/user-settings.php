...
if(filter_input(INPUT_POST,'submit_settings'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_user_settings' ) ) die( __('Failed security check','profile-magic') );
    ...
    <div class="content">
      ...
      <div class="childfieldsrow" id="pm_show_delete_profile_html" style=" <?php if($dbhandler->get_global_option_value('pm_show_delete_profile',0)==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
        ...
        <div class="uiminput">
          ...
          <input id="pm_default_avatar" type="hidden" name="pm_default_avatar" class="icon_id" value="<?php echo $pm_default_avatar; ?>" />
          ...
          <br />
          <span class="pg_profile_image_container" style="<?php if($pm_default_avatar=='')echo 'display:none;';?>">
              <img src="<?php echo $pmrequests->pg_get_default_avtar_src();?>" class="pg_upload_image_preview pm_preview_img" width="50" />
        ...
        <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Default Cover Image','profile-magic' ); ?>
        </div>
        <div class="uiminput">
              
          <input id="pm_default_cover_image" type="hidden" name="pm_default_cover_image" class="cover_icon_id" value="<?php echo $pm_default_cover_image; ?>" />
          <input id="field_cover_icon_button" name="field_cover_icon_button" class="button" type="button" value="<?php _e('Upload Image','profile-magic');?>" />
          <br />
          <span class="pg_cover_image_container" style="<?php if($pm_default_cover_image=='')echo 'display:none;';?>">
              <img src="<?php echo $pmrequests->pg_get_default_cover_image_src();?>" class="pg_upload_cover_image_preview pm_preview_img" width="50" />
                                <?php //echo get_avatar($user->user_email,50,'',false,array('class'=>'pm-user','id'=>'pg_upload_image_preview'));?>
        ...
        <div class="uimnote"><?php _e("Displays this image when a user has not selected or removed his/ her cover image.",'profile-magic');?></div>
      </div>
