...
<div class="uimrow" id="paidgroup">
  <div class="uimfield">
    <?php _e( 'Membership Charge','profile-magic' ); ?>
  </div>
  <div class="uiminput">
  ...
  <div class="uimnote"><?php _e('Charge registering users a one time fee for joining this group. Only works if you have selected Default option in Group Registration Form below. For custom registration forms, you can add more complex payment plans in respective form’s Dashboard.','profile-magic');?> </div>
</div>
<div class="childfieldsrow" id="paidgrouphtml" style=" <?php if(!empty($group_options) && isset($group_options['is_paid_group']) && $group_options['is_paid_group']==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
  <div class="uimrow">
    <div class="uimfield">
      <?php _e( 'Group Membership Price','profile-magic' ); ?>
    </div>
    ...
    <div class="uimnote"><?php _e('While signing up, users will be charged this amount for successful registration. User account will be automatically activated after successful payment. Make sure you have properly configured payment settings for this to work.','profile-magic');?> </div>
  </div>
</div>
...
<div class="uimrow">
  <div class="uimfield">
    <?php _e( 'Group Registration Form','profile-magic' ); ?>
  </div>
  <div class="uiminput">
      <select name="group_options[pg_rm_form]" id="pg_rm_form">
          <option value="0"><?php _e('ProfileGrid Default Group Form','profile-magic');?></option>
          <?php 
  ...
  <div class="uimnote"><?php echo sprintf(__("Select a user registration form for this group. ProfileGrid Default Group Form option will automatically create a form using profile fields in this group. Other form options are fetched from RegistrationMagic. For more information <a href='%s' target='_blank'>CLICK HERE</a>","profile-magic"),$pg_url);?> </div>
</div>
