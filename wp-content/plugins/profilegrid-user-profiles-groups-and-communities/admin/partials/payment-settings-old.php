...
if(filter_input(INPUT_POST,'submit_settings'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_payment_settings' ) ) die( __('Failed security check','profile-magic') );
...
<div class="uimagic">
  <form name="pm_user_settings" id="pm_user_settings" method="post">
    ...
    <div class="content">
      ...
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Payment Processor:','profile-magic' ); ?>
        </div>
        <div class="uiminput">
           <select name="pm_payment_method" id="pm_payment_method">
           <option value="paypal" selected><?php _e('PayPal','profile-magic');?></option>
           </select>
        </div>
        <div class="uimnote"> <?php _e('Select the payment system(s) you want to use for accepting payments. Make sure you configure them right.','profile-magic');?> </div>
      </div>
