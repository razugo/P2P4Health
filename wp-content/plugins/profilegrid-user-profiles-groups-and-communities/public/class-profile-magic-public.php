...
class Profile_Magic_Public {
		...
		wp_enqueue_script( $this->profile_magic, plugin_dir_url( __FILE__ ) . 'js/profile-magic-public.js', array( 'jquery' ), $this->version, false );
                ...
                $error['required_comman_field'] = __('Please fill all the required fields.','profile-magic');
                ...
                $error['user_exist'] = __('Sorry, username already exists.','profile-magic');
                $error['email_exist'] = __('Sorry, email already exists.','profile-magic');
                $error['show_more'] = __('More...','profile-magic');
                $error['show_less'] = __('Show less','profile-magic');
                $error['user_not_exit'] = __('Username does not exists.','profile-magic');
                $error['password_change_successfully'] =  __('Password changed Successfully','profile-magic');
                $error['allow_file_ext'] = $dbhandler->get_global_option_value('pm_allow_file_types','jpg|jpeg|png|gif');	
                $error['valid_phone_number'] = __('Please enter a valid phone number.','profile-magic');
        	$error['valid_mobile_number'] = __('Please enter a valid mobile number.','profile-magic');
                $error['valid_facebook_url'] = __('Please enter a valid Facebook url.','profile-magic');
                $error['valid_twitter_url'] = __('Please enter a Twitter url.','profile-magic');
                $error['valid_google_url'] = __('Please enter a valid Google url.','profile-magic');
                $error['valid_linked_in_url'] = __('Please enter a Linked In url.','profile-magic');
                ...
                $error['crop_alert_error'] = __('Please select a crop region then press submit.','profile-magic');
                $error['admin_note_error'] = __('Unable to add an empty note. Please write something and try again.','profile-magic');
                $error['empty_message_error'] = __('Unable to send an empty message. Please type something.','profile-magic');
                $error['invite_limit_error'] = __('Only ten users can be invited at a time.','profile-magic');
                $error['no_more_result'] = __('No More Result Found','profile-magic');
                $error['delete_friend_request'] = __('This will delete friend request from selected user(s). Do you wish to continue?','profile-magic');
                $error['remove_friend'] = __('This will remove selected user(s) from your friends list. Do you wish to continue?','profile-magic');
                $error['accept_friend_request_conf'] = __('This will accept request from selected user(s). Do you wish to continue?','profile-magic');
                $error['cancel_friend_request'] = __('This will cancel request from selected user(s). Do you wish to continue?','profile-magic');
                $error['next'] = __('Next','profile-magic');
                $error['back'] = __('Back','profile-magic');
                $error['submit'] = __('Submit','profile-magic');
                $error['empty_chat_message'] = __("I am sorry, I can't send an empty message. Please write something and try sending it again.",'profile-magic');
        ...
        public function profile_magic_add_blog($content)
        {
            ...
            if($dbhandler->get_global_option_value('pm_enable_blog','1')==1):
               ...
               return  '<div class="pm-login-box-error">'.__('Admin has disabled blog submissions. You cannot submit a new blog post at the moment.','profile-magic').'</div>';
            endif;
        ...
        public function profile_magic_password_reset_form( $attributes, $content = null ) 
        {
                ...
                if ( is_user_logged_in() ) 
                {
                    return $this->profile_magic_get_template_html( 'profile-magic-password-reset-form', $content, $attributes );
                        //return __( "You can reset your password by accessing Change Password tab in your profile's Settings section.",'profile-magic' );
                } 
                else 
                {
                        ...
                        {
                                return __( 'Invalid password reset link.','profile-magic' );
                        }
                }
        }
                ...
                exit;
                        ...
                        if ( isset( $_POST['pass1'] ) ) 
                        {
                                ...
                                exit;
                                 ...
                                 _e("Invalid request.","profile-magic");
                        }
        ...
        {
                // Create new message
                $msg  = __( 'Hello!','profile-magic' ) . "\r\n\r\n";
                $msg .= sprintf( __( 'You asked us to reset your password for your account using the email address %s.','profile-magic' ), $user_login ) . "\r\n\r\n";
                $msg .= __( "If this was a mistake, or you didn't ask for a password reset, just ignore this email and nothing will happen.",'profile-magic' ) . "\r\n\r\n";
                $msg .= __( 'To reset your password, visit the following address:','profile-magic' ) . "\r\n\r\n";
                $msg .= site_url( "wp-login.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . "\r\n\r\n";
                $msg .= __( 'Thanks!','profile-magic' ) . "\r\n";
        ...
        public function profile_magic_login_notice( $message ) 
        {
            ...
            $pmrequests = new PM_request;
                if ( isset( $_GET['disabled'] ) && $_GET['disabled'] == 1 ) 
                        $message =  '<div id="login_error">' . __( 'Account disabled','profile-magic') . '</div>';
                if ( isset( $_GET['errors'] )) 
                        $message =  '<div id="login_error">' .$pmrequests->profile_magic_get_error_message( filter_input(INPUT_GET, 'errors', FILTER_SANITIZE_STRING),'profile-magic'). '</div>';
                if ( isset( $_GET['activated'] ) && $_GET['activated']=='success') 
                        $message =  '<div class="message">' .__('Your account has been successfully activated.','profile-magic'). '</div>';
                return $message;
        }
                     ...
                     if(in_array($field->field_key,$match_fields)||$field->field_key==$match_fields){
                                ...
                                $ischecked = " ";
                                 ...
                                 if (isset($field_options['display_on_search']) && ($field_options['display_on_search'] == 1))
                                     {
                               $resp .=" <li class=\"pm-filter-item\"><input class=\"pm-filter-checkbox\" type=\"checkbox\" name=\"match_fields\" onclick=\"pm_advance_user_search()\" ".$ischecked." value=\"".$field->field_key."\" ><span class=\"pm-filter-value\">".__($field->field_name,'profile-magic')."</span></li>";
                        }
        ...
        public function pm_messenger_send_new_message()
        {
            ...
            if(isset($_POST)){
                ...
                echo $result;
                 ...
                 _e(" no post created","profile-magic");
            }
            die;
        }
   ...
   {  
            ...
            $user_info =array();
               ...
               if(isset($total_users)){
                $return .="<div  class=\"pm-all-members pm-dbfl pm-pad10\">"
                       . __('Total ','profile-magic')."<b>".$total_users
                       ."</b>". __(' members','profile-magic')."</div>";
               }
            ...
            foreach($users as $user)
                {       
                    ...
                    $return ="<div class=\"pm-message pm-dbfl pm-pad10\">"
                            . __("Sorry, your search returned no results." ,'profile-magic')
                            ."</div>";
                }
        ...
        public function pm_change_frontend_user_pass()
        {
                ...
                if(isset($current_user->ID) && !empty($_POST['pass1']))
                {
                        if(strlen($_POST['pass1'])<7)
                        {
                                $pm_error = __('Password is too short. At least 7 characters please!','profile-magic');
                        }
                        else
                        {
                                if($_POST['pass1']==$_POST['pass2'])
                                ...
                                {
                                        $pm_error = __('New Password and Repeat password does not match.','profile-magic');
                                }
                        }
                }
                else
                {
                        $pm_error = __('Password didn\'t changed.','profile-magic');
                }
                echo $pm_error;
                die;	
        }
        ...
        public function pm_submit_user_registration($post,$files,$server,$gid,$fields,$user_id,$textdomain)
        {
                ...
                if($pm_admin_notification==1)
                {
                        ...
                        $subject = __('New User Created','profile-magic');
                        $admin_message = '<p>'.__('New user created','profile-magic').'</p>'.$admin_html;
                ...
                else
                {
                    ...
                    {
                        $review_subject = $dbhandler->get_global_option_value('pm_account_review_email_subject',__('New user awaiting review','profile-magic'));
                        $review_body = $dbhandler->get_global_option_value('pm_account_review_email_body',__('{{display_name}} has just registered in {{group_name}} group and waiting to be reviewed. To review this member please click the following link: {{profile_link}}','profile-magic'));
       ...
       public function pm_submit_user_registration_paypal($post,$files,$server,$gid,$fields,$user_id,$textdomain)
        ...
        public function pg_create_post_type()
        {
            ...
            register_post_type( 'profilegrid_blogs',
                array(
                  'labels' => array(
                    'name' => __( ' User Blogs','profile-magic' ),
                    'singular_name' => __( 'User Blog','profile-magic')
                  ),
                  'public' => true,
                  'has_archive' => true,
...
//                  'menu_icon' =>'dashicons-testimonial'
        ...
        public function pm_show_friends_tab($uid,$gid)
	{
            ...
            {
                echo '<li class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg-friends">'. __('Friends','profile-magic').'</a></li>';
            }
	}
        ...
        public function pm_add_friend_request()
	{
		...
		?>
        <span><?php _e('Request Sent','profile-magic');?></span>
        ...
        public function pm_confirm_friend_request()
	{
		...
		echo '<b>'.__('Request Accepted!','profile-magic').'</b><br />'.__('You are now friends','profile-magic');	
		die;
	}
        ...
        public function pm_reject_friend_request()
	{
		...
		$dbhandler->update_row($identifier,'id',$requests->id,$data,array('%s','%d'),'%d');
                ...
                echo '<b>'.__('Request Rejected!','profile-magic').'</b><br />'. sprintf(__("You cancelled friend request from %s.","profile-magic"),$username2);
		die;
	}
        ...
        public function pm_block_friend()
	{
		...
		$requests = $pmfriends->profile_magic_is_exist_in_table($u1,$u2);
		$dbhandler->update_row($identifier,'id',$requests->id,$data,array('%s','%d'),'%d');
		echo '<b>'.__('Friend Blocked!','profile-magic').'</b><br />'.__('You have blocked this user','profile-magic');	
		die;
	}
        ...
        public function pm_unfriend_friend()
	{
		...
		if($cancel_request==1):
                ...
                echo '<b>'.__('Request Removed!','profile-magic').'</b>';
                else:
                         $username2 = $pmrequests->pm_get_display_name($u2);
                    echo '<b>'.__('Friend Removed!','profile-magic').'</b><br />'.sprintf(__("You have removed %s from your friend list.","profile-magic"),$username2);
                endif;
        ...
        public function pm_get_friends_notification()
	{
        ...
        public function pm_right_side_options($uid,$gid)
        {
            ...
            ?>
              <div class="pm-difr pm-pad20">
                  <a id="message_user" href="<?php echo $messenger_url; ?>" ><?php _e('Message','profile-magic');?></a>
            </div>
            <?php endif; 
        ...
        <div class="pmrow">
                ...
                <div class="pm-col">
                    ...
                    <div class="pm-field-lable">
                        <label for=""><?php _e('Price','profile-magic');?></label>
                    </div>
        ...
        <div class="pmrow">
                <div class="pm-col">
                    <div class="pm-form-field-icon"></div>
                    <div class="pm-field-lable">
                        <label for=""><?php _e('Payment Method','profile-magic');?><sup>*</sup></label>
                    </div>
                    <div class="pm-field-input pm_radiorequired">
                        <div class="pmradio">
                            <?php if($paypal_enable==1):?>
                            <div class="pm-radio-option"><input title="<?php _e('PayPal','profile-magic'); ?>" type="radio"  id="pm_payment_method" name="pm_payment_method" value="paypal" checked><?php _e('PayPal','profile-magic');?></div>
                            <?php endif;?>
        ...
        public function profile_magic_check_paypal_config($msg)
        {
            ...
            if($paypal_enable==1)
            {
                ...
                if($paypal_email=='')
                {
                    $msg = __('Oops! It looks like the PayPal payment system is not configured properly. Please check its settings.','profile-magic');
                }
                else
                {
                    $msg = '';
        ...
        public function pm_blocked_ips($args)
        {
            ...
            foreach($ips as $ip) 
            {
                ...
                if ( !empty( $ip ) && strpos($user_ip, $ip) === 0) 
                {
                    ...
                    {
                        return $pmrequests->profile_magic_get_error_message('blocked_ip','profile-magic');
                    }
                    else
                    {
                        $redirect_url = $pmrequests->profile_magic_get_frontend_url('pm_user_login_page',site_url('/wp-login.php'));
        ...
        public function pg_blocked_emails($args)
        {
            ...
            if ( isset( $useremail ) && is_email( $useremail ) ) 
            {
                ...
                if ( in_array($useremail, $emails ) )
                {
                    if( isset($args['form_type']) && $args['form_type']=='register')
                    {
                        return $pmrequests->profile_magic_get_error_message('blocked_email','profile-magic');
                    }
                    else
                    {
                        $redirect_url = $pmrequests->profile_magic_get_frontend_url('pm_user_login_page',site_url('/wp-login.php'));
                ...
                if ( in_array( $check_domain, $emails ) )
                {
                    if(isset($args['form_type']) && $args['form_type']=='register')
                    {
                        return $pmrequests->profile_magic_get_error_message('blocked_domain','profile-magic');
                    }
                    else
                    {
                        $redirect_url = $pmrequests->profile_magic_get_frontend_url('pm_user_login_page',site_url('/wp-login.php'));
        ...
        public function pm_check_blocked_word_during_registration($error,$post)
        {
           ...
           //echo $useremail;
            ...
            if ( $words != '' ) 
            {
                ...
                {
                    $error[] =  $pmrequests->profile_magic_get_error_message('blocked_words','profile-magic');
                }
            }
           return $error;
        }
        ...
        public function pm_account_deletion_notification($user_id)
        {
            ...
            {
                $subject = $dbhandler->get_global_option_value('pm_account_delete_email_subject',__('Account deleted','profile-magic'));
                $body = $dbhandler->get_global_option_value('pm_account_delete_email_body',__('{{display_name}} has just deleted their account.','profile-magic'));
        ...
        public function pg_user_profile_metadesc()
        {
            ...
            if(get_the_ID()==$dbhandler->get_global_option_value('pm_user_profile_page',0))
            {
                ...
                $avatar = get_avatar($user_info->user_email,150,'',false,array('class'=>'pm-user'));
        ...
        public function pm_show_settings_tab($uid,$gid)
	{
            ...
            {
                echo '<li class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg-settings">'. __('Settings','profile-magic').'</a></li>';
            }
	}
    ...
    public function pm_save_post_status()
    {
        ...
        if (!wp_verify_nonce($retrieved_nonce, 'save_pm_post_status' ) ) die( __('Failed security check','profile-magic') );
    ...
    public function pm_save_post_content_access_level()
    {
        ...
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'save_pm_post_content_access_level' ) ) die( __('Failed security check','profile-magic') );
    ...
    public function pm_save_edit_blog_post()
    {
        ...
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'save_pm_edit_blog_post' ) ) die( __('Failed security check','profile-magic') );
    ...
    public function pm_save_admin_note_content()
    {
        ...
        if (!wp_verify_nonce($retrieved_nonce, 'save_pm_admin_note_content' ) ) die( __('Failed security check','profile-magic') );
        if(is_numeric($postid))
        {
    ...
    public function pm_delete_admin_note()
    {
        ...
        $postid = filter_input(INPUT_POST, 'post_id');
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'delete_pm_admin_note' ) ) die( __('Failed security check','profile-magic') );
    ...
    public function pm_send_message_to_author()
    {
        ...
        $sid = $current_user->ID;
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'send_pm_message_to_author' ) ) die( __('Failed security check','profile-magic') );
        if(is_numeric($postid))
        {
            if($type=='blog')
            {
    ...
    public function pm_invite_user()
    {
        ...
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'invite_pm_user' ) ) die( __('Failed security check','profile-magic') );
        $message = '';
        foreach($emails as $email)
        {
            ...
            if($user_id)
            {
                ...
                if(!in_array($gid,$gid_array))
                {
                    ...
                    $message .= '<div class="pg-invited-user-result pg-group-user-info-box pg-invitation-failed pm-pad10 pm-bg pm-dbfl">
                        <div class="pm-difl pg-invited-user">'.get_avatar($email,26).'</div>
                        <div class="pm-difl pg-invited-user-info">
                            <div class="pg-invited-user-email pm-dbfl">'.$email.' &nbsp;</div>
                            <div class="pm-dbfl">'. __('User added to the group','profile-magic').'
                                <div class="pm-difr"><a href="'.$profile_url.'" target="_blank">'.__('View Profile','profile-magic').'</a></div>
                            </div>
                        </div>
                    </div>';
                }
                else
                {
                    ...
                    $group_link = $pmrequest->profile_magic_get_frontend_url('pm_group_page','');
                   ...
                   $message .=' <div class="pg-invited-user-result pg-group-user-info-box pg-invitation-failed pm-pad10 pm-bg pm-dbfl">
                        ...
                        <div class="pm-difl pg-invited-user-info">
                           <div class="pg-invited-user-email pm-dbfl">'.$email.' &nbsp;</div>
                            <div class="pm-dbfl">'.__("The user you are trying to add is already a member of this group",'profile-magic').'
                                <div class="pm-difr"><a href="'. $profile_url.'" target="_blank">'. __('View Profile','profile-magic').'</a></div>
                            </div>
                        </div>
                    </div>';
            ...
            else
            {
                ...
                $message .='<div class="pg-invited-user-result pg-group-user-info-box pg-invitation-success  pm-pad10 pm-bg pm-dbfl">
                        ...
                        <div class="pm-difl pg-invited-user-info">
                                <div class="pg-invited-user-email pm-dbfl">'.$email.' &nbsp;</div>
                            <div class="pm-dbfl">'.__('Invitation sent successfully.','profile-magic').'</div>
                        </div>
                    </div>';
    ...
    public function pm_remove_user_from_group()
    {
        ...
        if (!wp_verify_nonce($retrieved_nonce, 'remove_pm_user_from_group' ) ) die( __('Failed security check','profile-magic') );
    ...
    public function pm_deactivate_user_from_group()
    {
        ...
        $gid = filter_input(INPUT_POST, 'gid');
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'deactivate_pm_user_from_group' ) ) die( __('Failed security check','profile-magic') );
        if(is_numeric($user_id))
        {
    ...
    public function pm_reset_user_password()
    {
        ...
        $retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
        if (!wp_verify_nonce($retrieved_nonce, 'reset_pm_user_password' ) ) die( __('Failed security check','profile-magic') );
        ...
        if($is_group_limit==1)
        {
            ...
            ?>
                <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                    <?php echo __('User Limit Reached','profile-magic');?>
                ...
                <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                    <div class="pmrow">  
                        <div class="pm-col">
                            <p><?php _e(sprintf('%s',$message) ,'profile-magic'); ?> </p>         
                        </div>
                    </div>            
                </div>
    ...
    public function pg_rm_registration_tab($uid,$gid)
    {
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $title = $dbhandler->get_global_option_value('pm_rm_registrations_title',__('Registration','profile-magic'));
    ...
    public function pg_rm_payment_tab($uid,$gid)
    {
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $title = $dbhandler->get_global_option_value('pm_rm_payments_title',__('Payment History','profile-magic'));
    ...
    public function pg_rm_inbox_tab($uid,$gid)
    {
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $title = $dbhandler->get_global_option_value('pm_rm_inbox_title',__('Inbox','profile-magic'));
    ...
    public function pg_rm_orders_tab($uid,$gid)
    {
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $title = $dbhandler->get_global_option_value('pm_rm_orders_title',__('Orders','profile-magic'));
    ...
    public function pg_rm_downloads_tab($uid,$gid)
    {
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $title = $dbhandler->get_global_option_value('pm_rm_downloads_title',__('Downloads','profile-magic'));
    ...
    public function pg_rm_addresses_tab($uid,$gid)
    {
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $title = $dbhandler->get_global_option_value('pm_rm_addresses_title',__('Addresses','profile-magic'));
