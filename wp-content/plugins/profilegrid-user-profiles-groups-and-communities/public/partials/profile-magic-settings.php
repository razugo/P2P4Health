...
{
    $delete_error = __("You entered incorrect password. Please try again.",'profile-magic');
}
do_action('profile_magic_update_frontend_user_settings',$_POST,$current_user->ID);
if(isset($_POST['pg_privacy_submit']))
{
    
    update_user_meta($current_user->ID,'pm_profile_privacy',$_POST['pm_profile_privacy']);	
    update_user_meta($current_user->ID,'pm_hide_my_profile',$_POST['pm_hide_my_profile']);	
...
if(isset($_POST['pm_delete_account']))
{
    ...
    if(wp_check_password($_POST['password'], $current_user->data->user_pass, $current_user->ID))
    {
    // remove user
        if ( is_multisite() ) 
        {
    ...
    else
    {
         ...
         wp_redirect( esc_url_raw( $redirect_url . '#pg-delete-account' ) );
         exit;
    }
}
if(isset($_POST['my_account_submit']))
{
    ...
    $isupdate = update_user_meta( $current_user->ID,'first_name',$_POST['first_name']);
    $isupdate = update_user_meta( $current_user->ID,'last_name',$_POST['last_name']);
        ...
        if (isset( $_POST['user_email'])) {
            ...
            if ($current_user->user_email != $_POST['user_email']) {  
                ...
                if (email_exists( $_POST['user_email'] )){
                    ...
                    $isupdate = true;
                    $current_user->user_email = esc_attr( $_POST['user_email'] );
                    $args = array(
                        'ID'         => $current_user->ID,
    ...
    wp_redirect( esc_url_raw($redirect_url.'#pg-settings') );
	exit;
...
<div class="pm-section pm-dbfl" id="pg-settings-container">
 ...
 <div class="pm-section-left-panel pm-section-nav-vertical pm-difl">
    ...
    <ul class="dbfl">
        <li class="pm-dbfl pm-border-bt pm-pad10"><a class="pm-dbfl" href="#pg-edit-profile"><?php _e('Account Details','profile-magic');?></a></li>
        <?php if($dbhandler->get_global_option_value('pm_show_change_password','1')==1):?>
        <li class="pm-dbfl pm-border-bt pm-pad10"><a class="pm-dbfl" href="#pg-change-password"><?php _e('Change Password','profile-magic');?></a></li>
        <?php endif;?>
        ...
        <li class="pm-dbfl pm-border-bt pm-pad10"><a class="pm-dbfl" href="#pg-privacy"><?php _e('Privacy','profile-magic');?></a></li>
        <?php endif;?>
        <?php if($dbhandler->get_global_option_value('pm_show_delete_profile','0')==1):?>
        <li class="pm-dbfl pm-border-bt pm-pad10"><a class="pm-dbfl" href="#pg-delete-account"><?php _e('Delete Account','profile-magic');?></a></li>
        <?php endif;?>
    </ul>
</div>
