...
{
    echo do_shortcode('[RM_Form id="'.$rmformid.'"]' );
}
else
{
...
if(isset($_POST['reg_form_submit']) || isset($_POST['pm_payment_method']))
{
	...
	if($check_captcha==true)
	{
		...
		if(empty($errors))
		{
                    if(is_user_logged_in())
                    {
                        ...
                        $pmrequests->pm_update_user_custom_fields_data($_POST,$_FILES,$_SERVER,$gid,$fields,$current_user_id);
                        $group_type = $pmrequests->profile_magic_get_group_type($gid);
                        $is_paid_group = $pmrequests->profile_magic_check_paid_group($gid);
                        do_action('profile_magic_join_group_registration_process',$_POST,$gid,$current_user_id);
                        if($is_paid_group=="0")
                        {
                            $pmrequests->profile_magic_join_group_fun($current_user_id,$gid,$group_type);
                        }
                        do_action('profile_magic_join_paid_group_process',$_POST,$gid,$current_user_id);
                    }
                    else
                    {
	...
	{
		$errors = __('captcha failed','profile-magic');	
	}
}
else
{
	...
	if($check==false)
	{
		$message =  __('Require Useremail field','profile-magic');
	}
	elseif($is_group_limit==1)
	{
	...
	}elseif($pmrequests->profile_magic_check_paid_group($gid)>0)
        {
            ...
            if($message == 'disabled')
            {
                $message = __('Payment system is not configured to accept payments. Please configure at least one payment processor for this to work.','profile-magic');
            }
        }
	else
	{
...
if(isset($content['type']) && $content['type']=='multipage' && !isset($_POST['reg_form_submit']) && !is_user_logged_in())
{
