               ...
               <div class="pg-user-group-head pm-dbfl pm-pad10 pm-border-bt">
                   <div class="pm-dbfl"><a href="<?php echo esc_url($group_url); ?>"><?php echo $group->group_name; ?></a>
                       ...
                       <ul class="pg-dropdown-menu">
                           <li><a href="<?php echo esc_url($group_url);?>"><?php _e('Group Page','profile-magic');?></a></li>
                           
                           <?php if(class_exists('Profilegrid_Group_Wall') && $dbhandler->get_global_option_value('pm_enable_wall','0')):?>
                           <li><a href="<?php echo esc_url($group_url);?>#pg-groupwalls"><?php _e('Group Wall','profile-magic');?></a></li>
                           <?php endif;?>
                           <?php if(class_exists('Profilegrid_Group_photos') && $dbhandler->get_global_option_value('pm_enable_photos','0')):?>
                           <li><a href="<?php echo esc_url($group_url);?>#pg-group-photos"><?php _e('Group Photos','profile-magic');?></a></li>
                           <?php endif;?>
                           <?php if(class_exists('Profilegrid_Admin_Power') &&  $pmrequests->pg_check_in_single_group_is_user_group_leader($uid,$group->id )):?>
                           <li><a href="<?php echo esc_url($group_url);?>#pg_group_setting"><?php _e('Manage Group','profile-magic');?></a></li>
                           <?php endif;?>
                           <li><a onclick="pg_edit_blog_popup('group','remove_group','<?php echo $uid; ?>','<?php echo $group->id; ?>')" class="pm-remove"><?php _e('Leave Group','profile-magic');?></a></li>
                       </ul>
                   </div>
                       <?php endif;?>
                   </div>
