...
class Profile_Magic_Activator {
	...
	public function create_pages() {
		$dbhandler = new PM_DBhandler;
                $pmrequest = new PM_request;
		$pages['PM_Registration'] = array('post_type' => 'page','post_title' => __('Registration','profile-magic'),'post_status' => 'publish','post_name' => 'pm_registration','post_content' => '[PM_Registration id="1"]');
		$pages['PM_Group'] = array('post_type' => 'page','post_title' => __('Default User Group','profile-magic'),'post_status' => 'publish','post_name' => 'pm_group','post_content' => '[PM_Group id="1"]');
		$pages['PM_Groups'] = array('post_type' => 'page','post_title' => __('All Groups','profile-magic'),'post_status' => 'publish','post_name' => 'pm_groups','post_content' => '[PM_Groups]');
		$pages['PM_Login'] = array('post_type' => 'page','post_title' => __('Login','profile-magic'),'post_status' => 'publish','post_name' => 'pm_login','post_content' => '[PM_Login]');
		$pages['PM_Profile'] = array('post_type' => 'page','post_title' => __('My Profile','profile-magic'),'post_status' => 'publish','post_name' => 'pm_profile','post_content' => '[PM_Profile]');
		$pages['PM_Forget_Password'] = array('post_type' => 'page','post_title' => __('Forgot Password','profile-magic'),'post_status' => 'publish','post_name' => 'pm_forgot_password','post_content' => '[PM_Forget_Password]');
		$pages['PM_Add_Blog'] = array('post_type' => 'page','post_title' => __('Submit New Blog Post','profile-magic'),'post_status' => 'publish','post_name' => 'pm_submit_blog','post_content' => '[PM_Add_Blog]');
                $pages['PM_Search'] = array('post_type' => 'page','post_title' => __('Search Users','profile-magic'),'post_status' => 'publish','post_name' => 'pm_search','post_content' => '[PM_Search]');
                $pages['PM_User_Blogs'] = array('post_type' => 'page','post_title' => __('User Blogs','profile-magic'),'post_status' => 'publish','post_name' => 'pm_user_blogs','post_content' => '[PM_User_Blogs]');
		// The Query
		foreach($pages as $key=>$page) {
			$string = '['.$key;
			$my_query = new WP_Query( array('post_type' => 'any','post_status' => 'publish','s' => $string,'fields'=>'ids'));
			if(empty($my_query->posts)) { $page_id[$key] = wp_insert_post($page); }
			else { $page_id[$key] = $my_query->posts[0]; }
		}
		foreach($page_id as $key=>$id) {
			...
			if($key=='PM_Add_Blog')$field = 'pm_submit_blog';
                        if($key=='PM_Search')$field = 'pm_search_page';
                        if($key=='PM_User_Blogs')$field = 'pm_user_blogs_page';
		...
		if($dbhandler->pm_count('GROUPS')==0) {
			$data = array(
                            'group_name'=>__('Default User Group','profile-magic'),
                            'associate_role'=>'subscriber',
                            'group_desc'=>__('This the default user group. All existing registered users are automatically assigned this group. Groups can be modified or deleted by the admin.','profile-magic'),
                            'show_success_message'=>1,
                            'success_message'=>__('Thank you for signing up.','profile-magic'));
			$arg = array('%s','%s','%s','%d','%s');
			$gid = $dbhandler->insert_row('GROUPS', $data,$arg);
			$section_data = array('gid'=>$gid,'section_name'=>__('Personal Details','profile-magic'),'ordering'=>$gid);
	...
	public function get_field_format_type_FIELDS($field) {
        ...
        public function create_default_email_templates()
        {
           ...
           if($is_created=='0')
           {
                $resutls = $dbhandler->get_all_result('EMAIL_TMPL','*', array('tmpl_name' =>__('User Account Activated','profile-magic')));
                if(empty($resutls) || $resutls==null)
                {
