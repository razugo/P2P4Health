...
class profile_magic_paypal_class{
	...
	public function submit_paypal_post() {
		$paypal_url = ($this->sendbox == '1') ? PM_SSL_SAND_URL : PM_SSL_P_URL;
                ob_start();
		echo "<html>\n";
		echo "<head><title>".__("Processing Payment...","profile-magic")."</title></head>\n";
		echo "<body onLoad=\"document.forms['paypal_form'].submit();\">\n";
		echo "<center><h2>".__("Please wait, your order is being processed and you will be redirected to the paypal website.","profile-magic")."</h2></center>\n";
		...
		echo "<center><br/><br/>".__("If you are not automatically redirected to paypal within 5 seconds...","profile-magic")."<br/><br/>\n";
		echo '<input type="submit" value="'.__("Click Here","profile-magic").'"></center>';
		echo "\n";
		echo "</form>\n";
		echo "</body></html>\n";
 ...
 */
	public function validate_ipn() {
		...
		if (! preg_match ( '/paypal\.com$/', $hostname )) {
			$this->ipn_status = __('Validation post isn\'t from PayPal','profile-magic');
			$this->log_ipn_results ( false );
			return false;
		}
		
		if (isset($this->paypal_mail) && strtolower ( $_POST['receiver_email'] ) != strtolower(trim( $this->paypal_mail ))) {
			$this->ipn_status = __("Receiver Email Not Match","profile-magic");
			$this->log_ipn_results ( false );
			return false;
		}
		
		if (isset($this->txn_id)&& in_array($_POST['txn_id'],$this->txn_id)) {
			$this->ipn_status = __("txn_id have a duplicate","profile-magic");
			$this->log_ipn_results ( false );
			return false;
		}
		...
		if (! preg_match("/VERIFIED/i",$this->ipn_response)) {
			$this->ipn_status = __('IPN Validation Failed','profile-magic');
			$this->log_ipn_results(false);   
			return false;
		} else {
			$this->ipn_status = __("IPN VERIFIED","profile-magic");
	...
	private function log_ipn_results($success) {
		...
		$this->ipn_status .= __("IPN Response from Paypal Server:","profile-magic"). '\r\n' . $this->ipn_response;
		$this->write_to_log ();
	}
	...
	public function send_report($subject) {
		...
		$body .= __("from ","profile-magic") . $this->ipn_data ['payer_email'] . __(" on ","profile-magic") . date ( 'm/d/Y' );
		$body .= __(" at ","profile-magic") . date ( 'g:i A' ) . "\n\n".__("Details:","profile-magic")."\n" . $this->ipn_status;
...
//		// add_field() function.
//		echo __("<h3>paypal_class->dump_fields() Output:</h3>","profile-magic");
//		echo "<table width=\"95%\" border=\"1\" cellpadding=\"2\" cellspacing=\"0\">
//            <tr>
//               <td bgcolor=\"black\"><b><font color=\"white\">".__("Field Name","profile-magic")."</font></b></td>
//               <td bgcolor=\"black\"><b><font color=\"white\">".__("Value","profile-magic")."</font></b></td>
//            </tr>"; 
//		ksort($this->fields);
...
//		echo "</table><br>"; 
	...
	private function debug($msg) {
		...
		$fh = fopen ( $myFile, 'w' ) or die ( __("Can't open debug file. Please manually create the 'debug.log' file and make it writable.","profile-magic") );
