...
class PM_Custom_Fields {
    ...
    public function pm_get_custom_login_field_username($textdomain = '') {
        ...
        <div class="pm-col">
            ...
            <div class="pm-field-lable">
                <label for="user_login"><?php _e('Username or Email','profile-magic'); ?></label>
            </div>
    ...
    public function pm_get_custom_login_field_password($textdomain = '') {
        ...
        <div class="pm-col">
            <div class="pm-field-lable">
                <label for="user_pass"><?php _e('Password','profile-magic'); ?></label>
            </div>
            <div class="pm-field-input pm_required">
    ...
    public function pm_get_custom_login_field_remember_me($textdomain = '') {
        ...
        <div class="pm-col">
            ...
            <div class="pm-field-input">
                ...
                <label for="remember_me"><?php _e('Remember Me','profile-magic'); ?></label>
                <div class="errortext" style="display:none;"></div>
            </div>
        </div>
        <?php
    ...
    public function pm_get_custom_form_field_paragraph($row, $value = '', $textdomain = '') {
        ...
        <div class="pm-col">
            ...
            <div class="pm-field-input">
                <p class="<?php if (!empty($field_options)) echo esc_attr($field_options['css_class_attribute']); ?>"><?php if (!empty($field_options)) echo $field_options['paragraph_text']; ?></p>
                <div class="errortext" style="display:none;"></div>
            </div>
        </div>
        <?php
        ...
        if (!empty($field_options) && !empty($field_options['radio_option_value'])) {
            foreach ($field_options['radio_option_value'] as $radio) {
                ...
                if ($radio == 'chl_other') {
                    ...
                    ?>
                                <div class="pm-radio-option"><input title="<?php echo esc_attr($row->field_desc); ?>" value="<?php echo $radio; ?>" name="<?php echo esc_attr($row->field_key) . '[]'; ?>" type="checkbox" class="<?php if (!empty($field_options)) echo esc_attr($field_options['css_class_attribute']); ?>" onClick="pm_show_hide(this, 'pm_otherbox')" <?php echo $checked; ?>> <?php echo 'Other'; ?></div>
                                <?php
            ...
            <div class="pm-field-input pm_fileinput <?php if ($row->is_required == 1) echo 'pm_repeat_required'; ?>">
                ...
                <div class="pm_repeat">
                    ...
                    <?php if ($dbhandler->get_global_option_value('pm_allow_multiple_attachments') == 1): ?>
                        <a class="add"><span onClick="pm_add_repeat(this)"><?php _e('Add','profile-magic'); ?></span></a>
                        <a class="removebutton"><span class="remove" onClick="pm_remove_repeat(this)"><?php _e('Remove','profile-magic'); ?></span></a>
                    <?php endif; ?>
                    <div class="errortext" style="display:none;"></div>
                </div>
    ...
    public function pm_get_custom_form_field_user_avatar($row, $value = '', $textdomain = '') {
        ...
        {
            $message = sprintf( __( 'Size Restrictions: Please make sure the image size is equal to or larger than %d by %d pixels.','profile-magic' ),$pg_profile_photo_minimum_width ,$pg_profile_photo_minimum_width);
        }
        else
        {
            $message = sprintf( __( 'Size Restrictions: Please make sure the image size is equal to or larger than %d by %d pixels and does not exceeds total size of %d bytes.','profile-magic' ),$pg_profile_photo_minimum_width ,$pg_profile_photo_minimum_width,$pg_profile_image_max_file_size);
        }
        if ($value != '')
            $row->is_required = 0;
        if ($row->field_options != "")
        ...
        <div class="pm-col">
            ...
            <div class="pm-field-input pm_fileinput <?php if ($row->is_required == 1) echo 'pm_repeat_required'; ?>">
                <div class="pm_repeat">
                    <input title="<?php echo esc_attr($row->field_desc); ?>" type="file" class="<?php if (!empty($field_options)) echo esc_attr($field_options['css_class_attribute']); ?>" id="<?php echo esc_attr($row->field_key); ?>" name="<?php echo esc_attr($row->field_key) . '[]'; ?>" data-filter-placeholder="<?php if (!empty($field_options)) echo trim(esc_attr($field_options['allowed_file_types'])); ?>" />
                    <div class="pm-field-note"><?php echo $message;?></div>
                    <div class="errortext" style="display:none;"></div>
                </div>
            ...
            <div class="pm-field-input <?php if ($row->is_required == 1) echo 'pm_repeat_required'; ?>">
                <?php foreach ($values as $val): ?>
                    <div class="pm_repeat">
                        ...
                        <a class="add"><span onClick="pm_add_repeat(this)"><?php _e('Add','profile-magic'); ?></span></a><a class="removebutton"><span class="remove" onClick="pm_remove_repeat(this)"><?php _e('Remove','profile-magic'); ?></span></a>
                        <div class="errortext" style="display:none;"></div>
                    </div>
        <?php endforeach; ?>
        ...
        <div class="pm-col">
            ...
            <div class="pm-field-input <?php if ($row->is_required == 1) echo 'pm_radiorequired'; ?>">
                <div class="pmradio pm_gender">
                    <div class="pm-radio-option"><input title="<?php echo esc_attr($row->field_desc); ?>" type="radio" class="<?php if (!empty($field_options)) echo esc_attr($field_options['css_class_attribute']); ?>" id="<?php echo esc_attr($row->field_key); ?>" name="<?php echo esc_attr($row->field_key); ?>" value="<?php _e('Male', 'profile-magic') ?>" <?php if ($value == 'Male') echo 'checked'; ?>> <?php _e('Male', 'profile-magic') ?></div>
                    <div class="pm-radio-option"><input title="<?php echo esc_attr($row->field_desc); ?>" type="radio" class="<?php if (!empty($field_options)) echo esc_attr($field_options['css_class_attribute']); ?>" id="<?php echo esc_attr($row->field_key); ?>" name="<?php echo esc_attr($row->field_key); ?>" value="<?php _e('Female', 'profile-magic') ?>" <?php if ($value == 'Female') echo 'checked'; ?>> <?php _e('Female', 'profile-magic') ?></div>
                    <div class="pm-radio-option"><input title="<?php echo esc_attr($row->field_desc); ?>" type="radio" class="<?php if (!empty($field_options)) echo esc_attr($field_options['css_class_attribute']); ?>" id="<?php echo esc_attr($row->field_key); ?>" name="<?php echo esc_attr($row->field_key); ?>" value="<?php _e('Others', 'profile-magic') ?>" <?php if ($value == 'Others') echo 'checked'; ?>> <?php _e('Others', 'profile-magic') ?></div>
                </div>
                <div class="errortext" style="display:none;"></div>
            </div>
        </div>
 ...
 <div id="locationField">
      <input id="autocomplete" placeholder="<?php _e('Enter your address','profile-magic');?>"
             onFocus="" type="text"></input>
    </div>
<div>
     <input id="autocomplete" placeholder="<?php _e('Enter your address','profile-magic');?>" onFocus="" type="text"></input>
     <div>
         <input class="field" id="street_number" placeholder="<?php _e('street','profile-magic');?>" disabled="true"></input>
         <input class="field" id="route" placeholder="<?php _e('route','profile-magic');?>" disabled="true"></input>
         <input class="field" id="locality" placeholder="<?php _e('city','profile-magic');?>" disabled="true"></input>
         <input class="field" id="administrative_area_level_1" placeholder="<?php _e('state','profile-magic');?>" disabled="true"></input>
          <input class="field" id="postal_code" placeholder="<?php _e('zip code','profile-magic');?>" disabled="true"></input>
          <input class="field" id="country" placeholder="<?php _e('Country','profile-magic');?>" disabled="true"></input>     
     </div>
</div>
