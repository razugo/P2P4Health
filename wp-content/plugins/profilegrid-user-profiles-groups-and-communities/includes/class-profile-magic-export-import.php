...
class PM_Export_Import{
  ...
  public function pm_import_users_from_csv($post)
  {
    ...
    while($data = $file->fgetcsv( $delimiter )):
        ...
        if(isset($log['message']))$message .= '<li>'.$log['message'].'</li>';
        if(isset($log['status']) && $log['status']==0)$skipped = $skipped +1;
        if(isset($log['status']) && $log['status']==1)$created = $created +1;
        if(isset($log['status']) && $log['status']==2)$updated = $updated +1;
        unset($log);
        $i++;
    endwhile;
  
    
    echo '<div class="pm-import-result"><div class="pm-import-note">';
    echo '<strong>' . __('Import Process Complete','profile-magic') . '</strong><br />';
    echo '<i class="fa fa-user-plus" aria-hidden="true"></i> ' . __('New Users Created','profile-magic').' - '.$created.'<br/>' ;
    echo '<i class="fa fa-user" aria-hidden="true"></i> ' . __('Existing Users Updated','profile-magic').' - '.$updated.'<br/>' ;
    echo '<i class="fa fa-user-times" aria-hidden="true"></i> ' . __('Total Users Skipped','profile-magic').' - '.$skipped.'<br/></div>' ;
     echo '<b>' . __('Import Details','profile-magic') . '</b><br/>';
    echo '<div class="pm-import-details"><ul>';
    echo $message;
    echo '</ul></div></div>';
  }
  ...
  public function pm_create_new_field($gid,$sid,$heading)
  {
        ...
        $field_data = array('field_name'=>$field_name,'field_type'=>'text','field_options'=>$field_option,'field_icon'=>0,'associate_group'=>$gid,'associate_section'=>$sid,'show_in_signup_form'=>0,'is_required'=>0,'ordering'=>$ordering,'field_key'=>$field_key);
  ...
  public function pm_import_user($data,$post,$headings)
  {
    ...
    {
       $log['status'] = 0;
       $log['message'] = __('User is skipped due to empty record in CSV','profile-magic'); 
       return $log;
    }
    $dbhandler = new PM_DBhandler;
    $pmsanitizer = new PM_sanitizer;
    ...
    if($userexist)
    {
        ...
        else
        {
            // return log
            $log['status'] = 0;
            $log['message'] = sprintf(__('There\'s already a user with <b>%s</b> email. So we skipped it.','profile-magic'),$user_email);
            return $log;
        }
    }
    else
    {
        ...
        {
            $log['status'] = 0;
            $log['message'] = sprintf(__('Invalid Email format for User '.'<b>%s</b>','profile-magic'),$uemail);
            return $log;
        }
        if(is_numeric($user_id)):
            ...
            $log['status'] = 0;
            $log['message'] = sprintf(__('User not created with following username <b>%s</b> and email <b>%s<b>','profile-magic'),$user_name,$user_email);
            return $log;
        endif;
    }
    ...
    for($i=0;$i<count($data);$i++)
    {
       ...
       if($field_type=='address' && $data[$i]!='' )
       {
           ...
           {
               $gelocation = new Profilegrid_Geolocation_Public('profile-magic','1.0');
  ...
  public function pm_generate_mapping_table($post)
  {
        ...
        <div class="uimrow">
            <div class="uimfield">
                ...
                <div class="uimnote"><?php _e('Step 5: Select a section inside this Group where all imported fields will go. You can later redistribute the fields among different sections in Fields Manager.','profile-magic');?></div> 
        </div>

        <div class="uimrow">
            <div class="uimfield">
                  <?php _e( 'User Name','profile-magic' ); ?>
                </div>
                <div class="uiminput pm_select_required">
                    ...
                    {?>
                    <option value="<?php echo $key;?>"><?php echo $heading; ?></option>
                 <?php 
                    }
                    ?>
                    </select>
                ...
                <div class="uimnote"><?php _e('Step 6a: Map a column in your CSV to username field of this Group. New users will be assigned these usernames.','profile-magic');?></div> 
        </div>
        ...
        <div class="uimrow">
            <div class="uimfield">
                ...
                <div class="uimnote"><?php _e('Step 6b: Map a column in your CSV to User email field of this Group. Make sure the column contain email IDs.','profile-magic');?></div> 
        </div>
        ...
        <div class="pmagic-table">
                <table class="pg-import-table">
                    <tr class="pm_import_row">
                        <th><?php _e('CSV Mapping Scheme','profile-magic');?></th>
                        <th></th>
                        <th></th>
                        <th></th>
                    </tr>
        ...
        {
            ?>
                <tr class="pm_import_row" id="pm_import_row_<?php echo $i;?>">
                <td><i class="fa fa-check" aria-hidden="true"></i> <?php echo $heading;?></td>
                
                <td><input type="radio" name="pm_map_field_<?php echo $i;?>" value="0" onclick="pm_map_field_show_hide(0,'#pm_map_field_with_<?php echo $i;?>')" checked> <?php _e("Create new field","profile-magic");?></li></td>
                <td><input type="radio" name="pm_map_field_<?php echo $i;?>" value="1" onclick="pm_map_field_show_hide(1,'#pm_map_field_with_<?php echo $i;?>')"><?php _e("Map to an existing field","profile-magic");?></td>
                <td id="map_existing_field_html_<?php echo $i;?>">
                    <select name="pm_map_field_with_<?php echo $i;?>" id="pm_map_field_with_<?php echo $i;?>" style="display:none">
                    <?php 
                    foreach($fields as $field)
                    {?>
