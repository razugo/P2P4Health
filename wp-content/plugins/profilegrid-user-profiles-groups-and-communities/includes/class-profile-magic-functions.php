...
class Profile_Magic_Basic_Functions 
{
    ...
    public function null_field_notice() 
    {
       //echo $this->profile_magic;
         _e( 'Some of the options below require selecting fields from your form. Since you are creating new form from scratch, there are no fields in this form yet. You can come back later and modify these field specific options. You can safely ignore them for now and save the settings.','profile-magic' );
    }
    ...
    {
        $error = array();
		$error['pass_length'] = __('Your password should be at least 7 characters long.','profile-magic');
		$error['confirm_pass_not_match'] = __('Password and confirm password do not match.','profile-magic');
		$error['email_not_valid'] = __('Please enter a valid e-mail address.','profile-magic');
		$error['number_not_valid'] = __('Please enter a valid number.','profile-magic');
		$error['date_not_valid'] = __('Please enter a valid date (yyyy-mm-dd format)','profile-magic');
		$error['required_field'] = __('This is a required field','profile-magic');
		$error['file_type_not_valid'] = __('This file type is not allowed.','profile-magic');
		$error['number_not_valid'] = __('Please enter a valid number.','profile-magic');
		$error['number_not_valid'] = __('Please enter a valid number.','profile-magic');
		$error['number_not_valid'] = __('Please enter a valid number.','profile-magic');
		$error['number_not_valid'] = __('Please enter a valid number.','profile-magic');
    ...
    public function pg_get_extension_install_url($plugin)
    {
        switch($plugin)
        {
            ...
            case 'CUSTOM_TAB':
                ...
                break;
            case 'FRONTEND_GROUP_MANAGER':
                $url = "https://profilegrid.co/multi-group-users-and-group-manager/";
                break;
            default:
                $url = "http://profilegrid.co/extensions/";
                break;  
    ...
    public function pg_get_extension_button($plugin)
    {
        ...
        if($data->is_pg_extension_activate)
        {
            if($plugin !='FRONTEND_GROUP_MANAGER'):
        ?>
            <li>
            <a class="install-now button pg-install-now-btn" href="<?php echo $optionurl;?>"><?php _e("OPTIONS","profile-magic");?></a>
            </li>
        <?php
            endif;
        }
        elseif($data->is_pg_extension_installed)
        {
            ?>
            <li>
            <a class="install-now button pg-install-now-btn" href="<?php echo $data->pg_extsion_activate_url;?>"><?php _e("Activate Now","profile-magic");?></a>
            </li>
        <?php
        }
        else
        {
            if($plugin=='CUSTOM_PROFILE_SLUG' || $plugin=='DISPLAY_NAME' || $plugin=='BBPRESS' || $plugin=='WOOCOMMERCE')
            {
                $title = __('Free','profile-magic');
            }
            else
            {
                $title = __('More Info','profile-magic');
            }
            ?>
            <li>
            <a class="install-now button pg-install-now-btn" target="_blank" href="<?php echo $data->pg_get_extension_install_url;?>"><?php echo $title;?></a>
            </li>
            <?php 
        }
    }
    ...
    public function pg_get_extension_shortcode($plugin)
    {
        ...
        if($data->is_pg_extension_installed)
        {
            $link =  '<a class="pg_shortcode_link" href="'.$data->pg_extsion_activate_url.'">'.__("Activate Now","profile-magic").'</a>';

        }
        else
        {
           
            $link = '<a class="pg_shortcode_link" target="_blank" href="'.$data->pg_get_extension_install_url.'">'.__("More Info","profile-magic").'</a>';
