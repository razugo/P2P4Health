...
class PM_HTML_Creator
{
	...
	public function get_custom_fields_html($gid,$fields,$col=1)
	{
		...
		if($lastRec=count($fields))
		{
			...
			foreach($fields as $field) 
			{
				...
				$pm_customfields->pm_get_custom_form_fields($field,'','profile-magic');
				$i++;
			}
			echo '</div>';
		}
	...
	public function get_custom_fields_html_singlepage($gid,$fields,$col=1,$value='')
	{
        ...
        {
            $exclude = "and field_type not in('user_name','user_avatar','user_pass','confirm_pass','paragraph','heading')";
        }
        else{$exclude = '';}
        ...
        $filled_section = $dbhandler->get_all_result('FIELDS','associate_section',array('associate_group'=>$gid,'show_in_signup_form'=>1),'results',0,false,null,false,'','OBJECT',true);
		...
		$j=1;
                foreach($sections as $section)
                {
                        $fields =  $dbhandler->get_all_result('FIELDS', $column = '*',array('associate_group'=>$gid,'associate_section'=>$section->id,'show_in_signup_form'=>1),'results',0,false, $sort_by = 'ordering',false,$exclude);
                        ...
                        if(isset($fields) && !empty($fields))
                        {
                                ...
                                foreach($fields as $field)
                                {
                                    ...
                                    { 
                                       ...
                                       echo '<div class="pmrow">';
                                       $pm_customfields->pm_get_custom_form_fields($field,$field_value,'profile-magic');
                                       echo '</div>';	 
                                }
                                if($count == $j)
                                {
        ...
        <div class="pm-user-card pm-difl pm-border pm-radius5">
          ...
          <div class="pm-user-image pm-dbfl pm-bg-lt <?php echo $class;?>"><?php echo get_avatar($uid,$imgsize,'',false,$arg);?></div>
            <div class="pm-user-description pm-dbfl pm-bg-lt">
                ...
                <a href="<?php echo esc_url( $profile_url ); ?>"><?php echo $pmrequests->pm_get_display_name($uid);?> </a>
                </div>
                  <?php 
	...
	public function pm_field_captcha_error($errors)
	{
		...
		<div class="errortext pm_captcha_error">
    	     <?php  _e( 'Sorry, you didn\'t enter the correct captcha code.','profile-magic' ); ?>
         </div>
		<?php	
	}
       ...
       public function pm_get_user_blog_posts($uid,$pagenum=1,$limit=10)
        {
            ...
            if($post_type=='both'){$post_type = array('profilegrid_blogs','post');}
            ...
            if($pagenum<=$num_of_pages)
            {
                    ...
                    <div class="pm-blog-post-wrap pm-dbfl">
                        <div class="pm-blog-img-wrap pm-difl">
                            ...
                            <div class="pm-blog-status pm-difl">
                                <span class="pm-blog-time "><?php printf( _x( '%s ago', '%s = human-readable time difference','profile-magic' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) ); ?></span>
                ...
                ?>
                    <div class="pg-load-more-container pm-dbfl">
                        ...
                        <input type="submit" class="pm-load-more-blogs" onclick ="load_more_pg_blogs('<?php echo $uid;?>')" value="<?php _e('Load More','profile-magic'); ?>" />
                    </div>
                <?php
                endif;
            ...
            else
            {
                ...
                if($uid == $current_user->ID)
                {
                    echo "<div class='pg-alert-warning pg-alert-info'> ";
                     _e("You have not written any blog posts yet. Once you do, they will appear here.",'profile-magic');
                    echo "</div>";
                }
                else
                {
                    echo "<div class='pg-alert-warning pg-alert-info'>";
                    echo sprintf(__("Sorry, %s has not made any blog posts yet.",'profile-magic'),$displayname);
                    echo "</div>";
                }
    ...
    <path d="M0-.5h24v24H0z" fill="none"/>
            ...
            <div class="pm-section-left-panel pm-section-nav-vertical pm-difl " id="thread_pane">
                ...
                <div class="dbfl pm-new-message-area"><a title="Click here to compose the message" id="new_message_btn" onclick="create_new_message()"><i class="fa fa-plus" aria-hidden="true"></i><?php _e('New Message','profile-magic');?></a></div>
...
<div class="pm-section-right-panel">
            <div class="pm-blog-desc-wrap pm-difl pm-section-content pm-message-thread-section">
                ...
                <div class="pm-dbfl pm-chat-messenger-box">
                    ...
                    <textarea id="messenger_textarea" data-emojiable="true"  name="content" style="min-width: 100%;height:100px;"
                               ...
                               form="chat_message_form" placeholder="<?php _e("Type your message..","profile-magic");?>" ></textarea> 
                    <input type="hidden" disabled  maxlength="4" size="4" value="1000" id="counter">
                    <input type="hidden" name="sid" value="" />   
                    <div class="pm-messenger-button">
                        <label>
                          <input id="send_msg_btn" type="submit" name="send" value="<?php _e("send","profile-magic");?>"/>
    ...
    <g transform="matrix(1.05995e-15,17.3103,-17.3103,1.05995e-15,22248.8,-22939.9)">
            ...
            switch ($view) {
                case 1:
                    ...
                    <div class="pm-friend-action-bar pm-dbfl">
                        <button class="pm-difr pm-delete" onclick="pm_multiple_friends_remove('<?php echo $u1;?>')"><?php _e('Remove','profile-magic');?></button>
                    </div>
                    <?php
                    break;
                case 2:
                    ?>
                    <div class="pm-friend-action-bar pm-dbfl">
                        <button class="pm-difr pm-delete" onclick="pm_multiple_friends_request_delete('<?php echo $u1;?>')"><?php _e('Delete','profile-magic');?></button>
                        <button class="pm-difr " onclick="pm_multiple_friends_request_accept('<?php echo $u1;?>')"><?php _e('Accept','profile-magic');?></button>
                    </div>
                    <?php
                    break;
                case 3:
                    ?>
                    <div class="pm-friend-action-bar pm-dbfl">
                        <button class="pm-difr pm-delete" onclick="pm_multiple_friends_request_cancel('<?php echo $u1;?>')"><?php _e('Cancel','profile-magic');?></button>
                    </div>
                    <?php
                    break;
        ...
        public function pm_get_my_friends_html($uid,$pagenum,$pm_f_search,$limit,$view=1)
        {
            ...
            switch($view)
            {
                case 1:
                    ...
                    {
                        $error = __('<div class="pg-alert-warning pg-alert-info">You do not have any friends yet. You can add friends by sending friendship requests to people from their user profiles.</div> ','profile-magic');
                    }
                    else
                    {
                        $display_name = $pmrequests->pm_get_display_name($uid);
                        $error = sprintf(__('<span class="pg-alert-warning pg-alert-info">%s does not have any friends yet.</span> ','profile-magic'),$display_name);     
                    }
                     break;
                case 2:
                     $myfriends = $pmfriends->profile_magic_my_friends_requests($uid);
                     $error = __('<div class="pg-alert-warning pg-alert-info">No friend requests waiting for response</div>','profile-magic');
                    break;
                case 3:
                     $myfriends = $pmfriends->profile_magic_my_friends_requests($uid,1);
                    $error = __('<div class="pg-alert-warning pg-alert-info">No pending friend requests.</div> ','profile-magic');
                    break;
            }
        ...
        <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                        ...
                        <div class="pm-field-input">
                           <div class="pmradio">
                               <div class="pm-radio-option">
                                ...
                                <?php _e(sprintf('Select blogs on this page(%s)',$single),'profile-magic');?>
                               </div>
                                <div class="pm-radio-option">
                                <input type="radio" name="pm_blog_select_type" id="pm_blog_select_type" value="all">
                                <?php _e(sprintf('Select All(%s)',$total),'profile-magic');?>                
                                </div>
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_select_blog_posts()"><?php _e('Select','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
        ...
        public function change_blog_status_popup($post_id)
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php _e('Change Blog Post Status','profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_change_post_status_form" id="pg_change_post_status_form">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_post_status()"><?php _e('Update','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
        ...
        public function change_blog_status_success_popup($post_status)
        {
            ...
            {
                $title = __('Success!','profile-magic');
                $content = __(sprintf('Status of<b> %d</b> blog post(s) was changed successfully.',$post_status['count']),'profile-magic');
            }
            else
            {
            ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Success!','profile-magic');
                $content = __(sprintf('The status of post was successfully changed to %s ',$post_status),'profile-magic');
            }
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php if($post_status=='failed'):
                        _e("Something went wrong. Please try again or contact the admin.","profile-magic");
                        else:
                        echo $content;
                        endif;
                        ?>         
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Close','profile-magic');?></a></div> 
            </div>
        ...
        public function change_blog_access_control_popup($post_id,$gid)
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php _e('Access Control','profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_change_post_content_access_level" id="pg_change_post_content_access_level">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">
                <div class="pmrow pg-info-message">        
                   
                        <?php _e('The post will be accessible to:','profile-magic');?>
               ...
               <div class="pmradio">
                   <div class="pm-radio-option">
                    ...
                    <label> <?php _e('Public','profile-magic' ); ?></label>
                   </div>
                    <div class="pm-radio-option">
                    <input type="radio" name="pm_content_access" id="pm_content_access" value="2" <?php checked('2',$pm_content_access); ?> />
                    <label> <?php _e('Logged In Users','profile-magic' ); ?></label> 
                   </div>
                   
                   <div class="pm-radio-option">
                    <input type="radio" name="pm_content_access" id="pm_content_access" value="5" <?php checked('5',$pm_content_access); ?> />
                    <label> <?php _e('Only Group Members','profile-magic' ); ?></label> 
                   </div>
                   
                    <div class="pm-radio-option">
                    <input type="radio" name="pm_content_access" id="pm_content_access" value="3" <?php checked('3',$pm_content_access); ?> />
                    <label> <?php _e("Only Author's Friends",'profile-magic' ); ?></label> 
                   </div>
                   
                    <!-- <div class="pm-radio-option">
                    <input type="radio" name="pm_content_access" id="pm_content_access" value="4" <?php checked('4',$pm_content_access); ?> />
                    <label> <?php _e('Only Me','profile-magic' ); ?></label> 
                   </div>-->
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_post_access_content()"><?php _e('Update','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
        ...
        public function change_blog_access_control_success_popup($post_status)
        {
            ...
            {
                $title = __('Success!','profile-magic');
                $content = __(sprintf('Access level of <b>%d</b> blog post(s) was changed successfully.',$post_status['count']),'profile-magic');
            }
            else
            {
                ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Success!','profile-magic');
                ...
                {
                    case 1:
                        $new_status = __("Public","profile-magic");
                        break;
                    case 2:
                        $new_status = __("Logged in users","profile-magic");
                        break;
                    case 3:
                        $new_status = __("Only Author's Friends","profile-magic");
                        break;
                    case 5:
                        $new_status = __("Only Group Members","profile-magic");
                        break;
                    default:
                        $new_status = __("Public","profile-magic");
                        break;  
                }
                $content = __(sprintf('The post will now be visible to %s',$new_status),'profile-magic');
            }
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        ...
                        //echo $content;
                         _e("You changed post's privacy level.","profile-magic");
                        endif;
                        ?>         
    ...
    'editor_class' => '','teeny' => false,'dfw' => false,'tinymce' => $tiny,'quicktags' => false
            ...
            if(isset($post)){$desc = $post->post_content;}
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php _e('Edit Post','profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_edit_blog_post" id="pg_edit_blog_post">
             ...
             <div class="pmrow">
               <div class="pg-users-send-box pm-dbfl">        
                    ...
                    <div class="pm-field-lable pm-difl">
                            <label for="blog_title"><?php _e('Title','profile-magic');?><sup class="pm_estric">*</sup></label>
                    </div>
                    <div class="pm-field-input pm_required pm-difl">
                          <input title="Enter your title" type="text" class="" maxlength="" value="<?php if(isset($post)){echo $post->post_title;}?>" id="blog_title" name="blog_title" placeholder="">
                          <div class="errortext" style="display:none;"></div>
                    </div>
            ...
            <div class="pm-field-lable pm-difl">
                    <label for="blog_description"><?php _e('Description','profile-magic');?></label>
              </div>
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_edit_blog_post()"><?php _e('Update','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
                ...
                <div class="pmrow">        
                    <div class="pm-col">
                        ...
                        _e("Something went wrong. Please try again or contact the admin.","profile-magic");
                         ...
                         _e('The post was updated successfully!','profile-magic');
                        endif;
                        ?>         
        ...
        public function add_admin_note_on_blog_post_popup($post_id,$gid)
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php _e("Manager Note",'profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_add_admin_note" id="pg_add_admin_note">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">
                <div class="pmrow">
                <div class="pg-users-send-box pm-dbfl">        
               
                        <div class="pm-field-lable pm-difl">
                                <label for="pm_admin_note_content"><?php _e('Content','profile-magic');?></label>
                        </div>
                        <div class="pm-field-input pm-difl">
                            <textarea name="pm_admin_note_content" id="pm_admin_note_content" maxlength="5000" size="5000" onkeyup="pg_count_left_charactors('pm_admin_note_content','pg_text_counter','{CHAR} <?php _e('characters left','profile-magic');?>','5000')"><?php if(isset($pm_admin_note_content))echo $pm_admin_note_content;?></textarea>          
                            <div class="errortext" style="display:none;"></div>
                            <div id="pg_text_counter"><?php _e(sprintf('%d characters left',$char_limit),'profile-magic');?></div>
                        </div>

                </div>
                <div class="pg-users-send-box pm-dbfl">        
                        <div class="pm-field-lable pm-difl">
                                <label for="pm_admin_note_position"><?php _e('Position:','profile-magic');?></label>
                        </div>
                        <div class="pm-field-input pm-difl">
               <div class="pmradio">
                   <div class="pm-radio-option pm-difl">
                    <input type="radio" name="pm_admin_note_position" id="pm_admin_note_position" value="top" <?php checked('top',$pm_admin_note_position); if($pm_admin_note_position=='')echo 'checked';?>/>
                <?php _e('Top','profile-magic' ); ?>
                   </div>
                    <div class="pm-radio-option pm-difl">
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <?php if($char_limit=='5000')
                {?>
                   <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_post_admin_note_content()"><?php _e('Add','profile-magic');?></a></div> 
                <?php
                }
                else
                {?>
                    <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_post_admin_note_content()"><?php _e('Update','profile-magic');?></a></div> 
                    <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_delete_admin_note_content()"><?php _e('Delete','profile-magic');?></a></div> 
        ...
        {
            $path =  plugins_url( '../public/partials/images/popup-close.png', __FILE__ );
            ($postid=='failed')?$title = __('Failed!','profile-magic'):$title = __('Confirm','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr" onclick="pg_edit_popup_close()">
         ...
         <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_delete_admin_note" id="pg_delete_admin_note">
            
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php 
                            _e('Please confirm you wish to delete this Manager Note from the post.' ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pm_delete_admin_note()"><?php _e('Yes','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('No','profile-magic');?></a></div> 
            </div>
                ...
                <div class="pmrow">        
                    <div class="pm-col">
                        ...
                        _e("Something went wrong. Please try again or contact the admin.","profile-magic");
                         ...
                         _e('The Manager Note was deleted successfully.','profile-magic');
                        endif;
                        ?>         
        ...
        public function save_admin_note_success_popup($post_status)
        {
            ...
            {
                $title = __('Success!','profile-magic');
                $content = __(sprintf('The Manager Note of <b>%d</b> blog post(s) was added successfully.',$post_status['count']),'profile-magic');
            }
            else
            {
            ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Success!','profile-magic');
                $content = __('The Manager Note was added successfully to the post.','profile-magic');
            }
            $path =  plugins_url( '../public/partials/images/popup-close.png', __FILE__ );
        ...
        public function send_message_to_author_popup($post_id,$gid,$type='blog')
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php _e("New Message",'profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_send_author_message" id="pg_send_author_message">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">
                <div class="pmrow">        
                    <div class="pg-users-send-box pm-dbfl">
                     
                        <div class="pm-field-lable pm-difl">
                                <label><?php _e('To:','profile-magic');?></label>
                        </div>
                        <div class="pm-field-input pm-difl">
                            <span class="pm-message-username"><?php echo '@'.$username;?></span>
                            <?php if($first_name!='' && $last_name!=''): ?>
                            <span class="pm-message-dn">(<?php echo $display_name; ?>)</span>
                            <?php endif;?>
                        </div>
                    </div>
                </div>
                <div class="pmrow">        
                    <div class="pg-users-send-box pm-dbfl">
                        ...
                        <div class="pm-field-lable pm-difl">
                                <label for="pm_author_message"><?php _e('Message:','profile-magic');?></label>
                        </div>
                        <?php if($type=='blog'):?>
                        <div class="pm-field-input pm-difl">
                           <textarea name="pm_author_message" id="pm_author_message" ><?php _e(sprintf('About your blog post: %s',$post->post_title),'profile-magic');?></textarea>          
                            <div class="errortext" style="display:none;"></div>
                            
                        </div>   
                        <?php else:?>
                        <div class="pm-field-input pm-difl">
                            <textarea name="pm_author_message" id="pm_author_message" placeholder="<?php _e('Type your message here','profile-magic');?>"></textarea>          
                            <div class="errortext" style="display:none;"></div>
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_submit_author_message()"><?php _e('Send','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
        ...
        public function author_msg_send_success_popup($post_status)
        {
            ...
            {
                $title = __('Success!','profile-magic');
                $content = __(sprintf('Your message was successfully sent to <b>%d</b> recipient(s).',$post_status['count']),'profile-magic');
            }
            else
            {
                ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Success!','profile-magic');
                $display_name = $pm_request->profile_magic_get_user_field_value($post_status,'display_name');
                $content = __(sprintf('Your Message to <b> %s </b> was sent successfully.',$display_name),'profile-magic');
            }
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
        ...
        public function add_user_in_group_popup($gid)
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php _e("Add User",'profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_add_user" id="pg_add_user">
                    <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">
                        <div class="pmrow"> 
                     <div class="pm-dbfl pg-info-message">        
                    
                                <?php
                                _e('You can simultaneously add upto <strong>10 users</strong> to your group using this method.', 'profile-magic');
                                ?>         
                    ...
                    <div class="group-popup-container">
                        <div class="pm-field-input pm-pad10 pm-dbfl">
                            <div class="pm_repeat pm-pad10">
                                ...
                                <a class="pg-add-user"><span onClick="pm_add_repeat(this)"><?php _e('Add More', 'profile-magic'); ?></span></a><a class="pg-remove-user pm-remove"><span class="remove" onClick="pm_remove_repeat(this)"><?php _e('Remove', 'profile-magic'); ?></span></a>
                                <div class="errortext" style="display:none;"></div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_invite_user()"><?php _e('Invite','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a onclick="pg_edit_popup_close()" class="pm-remove"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
        ...
        public function invitation_send_result_success_popup($result)
        {
            $path =  plugins_url( '../public/partials/images/popup-close.png', __FILE__ );
            $pm_request = new PM_request;
            ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Results','profile-magic');
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-dbfl pg-info-message">
                        <?php _e('Here are the results of User Invites you just sent.','profile-magic');?>         
                    </div>
                </div>
            ...
            <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('Close','profile-magic');?></a></div> 
            </div>
        ...
        public function remove_user_in_group_popup($id,$gid)
        {
            ...
            <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_remove_user_in_group" id="pg_remove_user_in_group">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php 
                            _e('Please confirm you really want to remove the user from the group.' ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pm_remove_user_from_group()"><?php _e('Yes','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('No','profile-magic');?></a></div> 
            </div>
        ...
        public function remove_admin_in_group_popup($id,$gid)
        {
            ...
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                 ...
                 <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_remove_admin" id="pg_remove_admin">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php 
                            $user = get_user_by('ID', $id);
                            _e(sprintf('You are going to remove <strong>%s</strong> from the Group Manager List. The user will still remain a member of the group. Do you wish to proceed?',$user->user_login) ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pm_remove_admin_from_group()"><?php _e('Yes','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('No','profile-magic');?></a></div> 
            </div>
        ...
        public function pm_remove_user_success_popup($post_status)
        {
            ...
            {
                $title = __('Success!','profile-magic');
                $content = __(sprintf('<b>%d</b> User(s) were removed from the group successfully.',$post_status['count']),'profile-magic');
            }
            else
            {
                ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Success!','profile-magic');
                $content = __('User successfully removed.','profile-magic');
        ...
        public function deactivate_user_popup($id,$gid)
        {
         ...
         <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_deactivate_user_in_group" id="pg_deactivate_user_in_group">
            
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php 
                            _e('Please confirm you really want to suspend the user from the group.' ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pm_deactivate_user_from_group()"><?php _e('Yes','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('No','profile-magic');?></a></div> 
            </div>
        ...
        public function pm_deactivate_user_success_popup($post_status)
        {
            ...
            {
                $title = __('Success!','profile-magic');
                $content = __(sprintf('<b>%d</b> User(s) were Suspended from the group successfully.',$post_status['count']),'profile-magic');
            }
            else
            {
                ($post_status=='failed')?$title = __('Failed!','profile-magic'):$title = __('Success!','profile-magic');
                $content = __('User successfully Suspended.','profile-magic');
        ...
        {
            $path =  plugins_url( '../public/partials/images/popup-close.png', __FILE__ );
            ($postid=='failed')?$title = __('Failed!','profile-magic'):$title = __('Reset Password','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr">
         ...
         <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_reset_user_password" id="pg_reset_user_password">
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap"> 
                ...
                <div class="pmrow">
                    <div class="pm-dbfl pg-info-message">
                        <?php 
                            _e('Enter a new password manually or autogenerate.' ,'profile-magic');
                       ?>         
                    </div>
                
                <div class="group-popup-container">
                     <div class="pm-field-input">
                         <input type='password' id='pm_new_pass' name='pm_new_pass' placeholder="<?php _e('Enter New Password','profile-magic');?>" onkeyup="pg_check_password_strenth()" />	
                     ...
                     <div class="pm-field-input pm-dbfr">
                         <a onclick="pg_password_auto_generate('pm_new_pass')"><?php _e('Autogenerate'); ?></a>
                     </div>  
                 ...
                 <div class="pg-email-password pm-dbfl">
                             ...
                             <div class="pm-field-input">
                                 ...
                                 <?php _e('Email new password to the user', 'profile-magic'); ?>
                                 </label>
                             </div>             
                         </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a id="pm_member_reset_password_link" class="pg-setting-disabled"><?php _e('Reset','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
         ...
         public function pm_reset_user_password_success_popup($name,$is_emailed)
        {
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php if($is_emailed):
                        _e(sprintf("Password for user <b>%s</b> was reset and emailed successfully.",$name),"profile-magic");
                        else:
                            _e( sprintf("Password for user <b>%s</b> was reset successfully.",$name),'profile-magic');
                        endif;
                        ?>         
        ...
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_change_post_status_form" id="pg_change_post_status_form">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                 ...
                 <div class="pmrow pg-info-message">        
                        ...
                        <?php _e(sprintf('You are going to change status of <b>%d</b> selected blog posts.',$count),'profile-magic');?>
        ...
        <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_change_post_content_access_level" id="pg_change_post_content_access_level">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">
                <div class="pmrow pg-info-message">        
                        ...
                        <?php _e(sprintf('Selected <b>%d</b> post(s) will be accessible to:',$count),'profile-magic');?>
                ...
                <div class="pmrow ">        
                    <div class="pg-users-send-box pm-dbfl">
                        ...
                        <div class="pm-field-input pm-difl">
                            <textarea name="pm_admin_note_content" id="pm_admin_note_content" maxlength="5000" size="5000" onkeyup="pg_count_left_charactors('pm_admin_note_content','pg_text_counter','{CHAR} <?php _e('characters left','profile-magic');?>','5000')"><?php if(isset($pm_admin_note_content))echo $pm_admin_note_content;?></textarea>          
                            <div class="errortext" style="display:none;"></div>
                            <div id="pg_text_counter"><?php _e(sprintf('<b>%d</b> characters left',$char_limit),'profile-magic');?></div>
                        </div>
                    </div>
                </div>
                <div class="pmrow">        
                ...
                <div class="pmrow">        
                    <div class="pg-users-send-box pm-dbfl">
                        ...
                        <div class="pm-field-input pm-difl pm-popup-title">
                            <?php _e(sprintf('%d Recipient(s)',$count),'profile-magic');?>
                        </div>
                    </div>
                </div>
                <div class="pmrow">        
        ...
        public function remove_bulk_user_in_group_popup($id,$gid)
        {
            ...
            $count = count($id);
            $pm_admin_note_content = '';
            $title = __('Remove Users','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr">
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                        <?php 
                            _e(sprintf('You are going to remove <b>%d</b> user(s). Do you wish to proceed?',$count) ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
        ...
        public function deactivate_bulk_user_popup($id,$gid)
        {
            ...
            $count = count($id);
            $pm_admin_note_content = '';
            $title = __('Suspend Users','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr">
         ...
         <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_deactivate_user_in_group" id="pg_deactivate_user_in_group">
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                         <?php 
                            _e(sprintf('You are going to suspend <b>%d</b> user(s). Do you wish to proceed?',$count) ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
        ...
        public function pg_remove_group_in_user_profile_popup($id,$gid)
        {
            ...
            //echo $group_manager_label;
            $path =  plugins_url( '../public/partials/images/popup-close.png', __FILE__ );
            ($postid=='failed')?$title = __('Failed!','profile-magic'):$title = __('Leave Group','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr">
            ...
            <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_remove_group_in_user_profile" id="pg_remove_group_in_user_profile">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">  
                    <?php if($is_leader):?>
                     <div class="pm-col">
                        <?php 
                            echo sprintf(__('You are currently %s of this group. Leaving the group will remove your %s privileges along with group membership. To regain %s privileges in future, another %s must assign you the role first. Do you wish to proceed?' ,'profile-magic'),$group_manager_label,$group_manager_label,$group_manager_label,$group_manager_label);
                       ?>         
                    </div>
                    <?php else:?>
                    <div class="pm-col">
                        <?php 
                            echo sprintf(__('You are about to leave %s group. You will no longer have access to Group updates or Group related information. Do you wish to continue?' ,'profile-magic'),$groupinfo->group_name);
                       ?>         
                    </div>
                    <?php endif;?>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_remove_user_group('<?php echo esc_attr($id);?>','<?php echo esc_attr($gid);?>')"><?php _e('Yes','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('No','profile-magic');?></a></div> 
            </div>
            <input type="hidden" name="action" id="action" value="pm_remove_user_from_group" />
             <?php wp_nonce_field('remove_pm_user_from_group'); ?>
        ...
        public function pg_decline_request_to_join_group_popup($id,$gid)
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo __('Decline Request','profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
         
            <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_decline_request_to_join_group" id="pg_decline_request_to_join_group">
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">  
                    <div class="pm-dbfl pg-info-message">
                        <?php 
                           _e('You are about to decline membership request by:' ,'profile-magic');
                       ?>         
                    </div>
                      </div>
                <div class="pmrow">  
                    <div class="pm-col">
                        <div class="pg-group-user-info-box  pm-pad10 pm-bg pm-dbfl">
                            ...
                            <div class="pg-group-user-view-link pm-difr">
                                <a href="<?php echo $pm_request->pm_get_user_profile_url($user->ID)?>" target="_blank"><?php _e('View Profile');?></a>
                            </div>
                            </div>
                            </div>
                        </div>     
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_decline_join_request('<?php echo esc_attr($id);?>','<?php echo esc_attr($gid);?>')"><?php _e('Decline','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
        ...
        public function pg_accept_request_to_join_group_popup($id,$gid)
        {
            ...
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo __('Approve Request','profile-magic');?>
                  <div class="pm-popup-close pm-difr">
                      <img src="<?php echo $path; ?>" height="24px" width="24px">
                  </div>
            </div>
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">  
                    <div class="pm-dbfl pg-info-message">
                        <?php 
                           _e('You are about to approve membership request by:' ,'profile-magic');
                       ?>         
                    </div>
                </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pg_approve_join_request('<?php echo esc_attr($id);?>','<?php echo esc_attr($gid);?>')"><?php _e('Approve','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
            <input type="hidden" name="action" id="action" value="pm_decline_join_request" />
             <?php wp_nonce_field('decline_pm_join_request'); ?>
        ...
        public function pg_join_paid_group_html($gid,$uid)
        {
            ...
            <form method="post" class="pmagic-form pm-dbfl">
                <fieldset>
                <legend><?php _e('Payment','profile-magic');?></legend>
                <?php do_action('profile_magic_custom_fields_html',$gid); ?>
                <div class="all_errors" style="display:none;"></div>
                <input type="hidden" name="pg_uid" id="pg_uid" value="<?php echo $uid;?>" />
                ...
                <input type="submit" name="pg_join_paid_group_form" id="pg_join_paid_group_form" class="pm_button" value="<?php _e('Join Group','profile-magic');?>" />
                </div>
            </form>
            </div>
            </div>
        ...
        public function pg_decline_request_bulk_to_join_group_popup($id,$gid)
        {
            ...
            $post_id = $pm_request->pm_encrypt_decrypt_pass('encrypt',maybe_serialize($id));
            $count = count($id);
            $title = __('Decline Requests','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr">
         ...
         <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_decline_bulk_join_group_requests" id="pg_decline_bulk_join_group_requests">
            
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                         <?php 
                            _e(sprintf('You are going to decline <b>%d</b> group membership user requests. Do you wish to proceed?',$count) ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pm_decline_bulk_join_group_requests()"><?php _e('Decline','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
         ...
         public function pg_accept_request_bulk_to_join_group_popup($id,$gid)
        {
            ...
            $post_id = $pm_request->pm_encrypt_decrypt_pass('encrypt',maybe_serialize($id));
            $count = count($id);
            $title = __('Approve Requests','profile-magic');
            ?>
            <div class="pm-popup-title pm-dbfl pm-bg-lt pm-pad10 pm-border-bt">
                <?php echo $title;?>
                  <div class="pm-popup-close pm-difr">
         ...
         <form action="<?php echo admin_url( 'admin-ajax.php' );?>" method="post" name="pg_decline_bulk_join_group_requests" id="pg_decline_bulk_join_group_requests">
            ...
            <div class="pm-dbfl pm-pad10 pg-group-setting-popup-wrap">  
                <div class="pmrow">        
                    <div class="pm-col">
                         <?php 
                            _e(sprintf('You are going to approve <b>%d</b> group membership user requests. Do you wish to proceed?',$count) ,'profile-magic');
                       ?>         
                    </div>
                </div>
            </div>
           ...
           <div class="pg-group-setting-popup-footer pm-dbfl">
                <div class="pg-group-setting-bt pm-difl"><a onclick="pm_approve_bulk_join_group_requests()"><?php _e('Approve','profile-magic');?></a></div> 
                <div class="pg-group-setting-bt pg-group-setting-close-btn pm-difl"><a class="pm-remove" onclick="pg_edit_popup_close()"><?php _e('Cancel','profile-magic');?></a></div> 
            </div>
            ...
            if($pagenum<=$num_of_pages)
            {
                ...
                ?>
                    <div class="pg-load-more-container pm-dbfl">
                        <div class="pm-loader" style="display:none;"></div>
                        <input type="hidden" id="pg_next_blog_page" value="<?php echo $pagenum + 1; ?>" />
                        <input type="submit" class="pm-load-more-blogs" onclick ="load_more_user_blogs_shortcode_posts('<?php echo $authors;?>','<?php echo $posttypes;?>')" value="<?php _e('Load More','profile-magic'); ?>" />
                    </div>
                <?php
                endif;
