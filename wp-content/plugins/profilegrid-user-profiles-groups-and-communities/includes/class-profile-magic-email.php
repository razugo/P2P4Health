...
class PM_Emails
{
	public function pm_send_group_based_notification($gid,$userid,$event='',$postid=false)
	{
        ...
        public function pm_send_group_based_notification_to_group_admin($gid,$userid,$event='',$leader)
	{
                $dbhandler = new PM_DBhandler;
		$row = $dbhandler->get_row('GROUPS',$gid);
		$tmpl_id = '';
        ...
        public function pm_send_remove_from_group_user_notification($uid,$gid)
	{
            ...
            $from_email_address = $pmrequests->profile_magic_get_from_email();
            $admin_email_address = $pmrequests->profile_magic_get_admin_email();
            $subject = __('Membership Terminated','profile-magic');
            $message = __("Hi,","profile-magic")."<br />\r\n\r\n";
            $message .= __("Your membership for the group {{group_name}} has been terminated by the {{group_admin_label}} on {{site_name}}. You no longer will have access to private areas of the group.","profile-magic");
            $message .= "<br />\r\n\r\n";	
	...
	public function pm_send_user_notification($id,$userid,$postid=false,$leader=false,$gid=false)
	{
            ...
            else
            {
                ...
                {
                    $additional_message =__("Note: You are receiving this email from {{group_name}} group since it is without a {{group_admin_label}}. Please define a new {{group_admin_label}} for this group by going to {{edit_group_url}} or consider making it an open group.","profile-magic");
                } 
	...
	public function pm_get_email_subject($id,$userid=false,$postid=false,$gid=false)
	{
        ...
        public function pm_send_invite_link($email_id,$gid)
        {
            ...
            $subject = sprintf(__('Invitation to Join Group - %s','profile-magic'),$group_name);
            ...
            $message = sprintf(__("Hello,<br /><br />You have been invited by a %s to join group %s. Please visit this link to sign up:<br /><br /> %s <br /><br /> Regards.","profile-magic"),$group_admin_label,$group_name,$registration_url);
        ...
        public function pm_send_unread_message_notification($sid,$rid)
	{
            $pmrequests = new PM_request;
            $dbhandler = new PM_DBhandler;
            $subject = $dbhandler->get_global_option_value('pm_unread_message_email_subject',__('New Private Message from {{sender_name}}','profile-magic'));
            $message = $dbhandler->get_global_option_value('pm_unread_message_email_body',__('Hi {{display_name}},<br /><br />You just received a new private message from {{sender_name}}. Visit your profile at {{profile_link}} to make sure you are not missing out on the latest updates.','profile-magic'));
