        ...
        if (!empty($threads)) {
            ...
            foreach ($threads as $thread) {
                ...
                if($thread_timestamp=='1 min'){
                    $thread_timestamp=__('just now','profile-magic');
                }else{
                    $thread_timestamp = $thread_timestamp.__(' ago','profile-magic');
                }
...
//            
        ...
        {
            $return = __("You have no conversations yet.","profile-magic");
        }
       ...
       if(!empty($messages)){
        ...
        if (!empty($messages)) {
            if (sizeof($messages) == $limit) {
                $return .= "<button   id=\"load_more_message\" pagenum=\"$loadnum\" >".__("Load More","profile-magic")."</button>";
            }
            foreach ($messages as $message)
            {
                if($message->m_id>=$last_mid)
                {
                    ...
                    $msg_timestamp=__('just now','profile-magic');
