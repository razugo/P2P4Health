...
$html_creator = new PM_HTML_Creator('profile-magic','1');
...
if ( isset( $_REQUEST['errors'] ) ) {
    ...
    foreach ( $error_codes as $error_code ) {
        $attributes['errors'][]=  $pmrequests->profile_magic_get_error_message($error_code,'profile-magic');
    }
}
...
if ( $attributes['lost_password_sent'] ) $pm_error .= '<span class="pm_info">'.__( 'Check your email for a link to reset your password.','profile-magic' ).'</span>';
if ( $attributes['password_updated'] ) $pm_error .= '<span class="pm_info">'.__( 'Your password has been changed. You can sign in now.','profile-magic' ).'</span>';
if ( $attributes['user_activated'] ) $pm_error .= '<span class="pm_info">'.__( 'Your account has been successfully activated.','profile-magic' ).'</span>';
...
if(isset($_POST['login_form_submit']))
{
	...
	if($check_captcha==true)
	{
		...
		if (!wp_verify_nonce($retrieved_nonce, 'pm_login_form' ) ) die( __('Failed security check','profile-magic') );
	...
	{
		$pm_error .= '<p class="pm_error">'.__( 'Captcha Failed','profile-magic' ).'</p>';	
	}
}
...
if ( is_user_logged_in()) : 
 ...
 <div class="pg-user-content pm-dbfl">
     <div class="pg-user-content-img pm-difl">
        ...
        <div id="pm-show-profile-image" class="pm-profile-image pm-difl pm-pad10"> <?php echo get_avatar($user_info->user_email,50,'',false,array('class'=>'pm-user'));?>
...
<!-----Form Starts----->
  ...
  <?php wp_nonce_field('pm_login_form'); ?>
      <div class="pm_widget-login-row pm-difl">
          <label for="<?php echo esc_attr('user_login');?>" class="pm-dbfl"><?php _e('Email or Username','profile-magic');?></label>
        <input type="text" name="<?php echo esc_attr('user_login');?>" id="<?php echo esc_attr('user_login');?>" placeholder="<?php _e('Email or Username','profile-magic');?>" required="required">
         </div>
            <div class="pm_widget-login-row pm-difl">
        <label for="<?php echo esc_attr('user_pass');?>" class="pm-dbfl"><?php _e('Password','profile-magic');?></label>
        <input type="password" name="<?php echo esc_attr('user_pass');?>" id="<?php echo esc_attr('user_pass');?>" placeholder="<?php _e('Password','profile-magic');?>" required="required">
            </div>
            <div class="pm-login-box-bottom-container pm-dbfl">
                <input type="submit" value="<?php _e('Login','profile-magic');?>" name="login_form_submit" class="">
                <div class="pm-login-links-box pm-dbfl">
                <a href="<?php echo $forget_password_url;?>"><?php _e('Forgot Password?','profile-magic');?></a>
                </div>
            </div>
   <?php
